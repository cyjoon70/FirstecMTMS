...
namespace CS.CSB011
{
    public partial class CSB011 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region Form Load시
        private void CSB011_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        private void detail_SearchExec(int item_su)
        {
            try
            {
                ...
                int colspan = 1;
                int colfirst = 3;
                fpSpread1.Sheets[0].ColumnCount = 3 + item_su;
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        fpSpread1.Sheets[0].ColumnHeader.Cells[0, 3 + col_idx].Text = dt.Rows[i]["COST_CLASS_NM"].ToString();
                        fpSpread1.Sheets[0].ColumnHeader.Cells[1, 3 + col_idx].Text = dt.Rows[i]["COST_ELEMENT_NM"].ToString();

                        if (dt.Rows[i]["COST_CLASS_NM"].ToString() == dt.Rows[i]["COST_ELEMENT_NM"].ToString())
                            fpSpread1.Sheets[0].ColumnHeader.Cells[0, 3 + col_idx].RowSpan = 2;
                        else
                            fpSpread1.Sheets[0].ColumnHeader.Cells[1, 3 + col_idx].Text = dt.Rows[i]["COST_ELEMENT_NM"].ToString();
                        ...
                        if (div1 == "B03")
                        {
                            ...
                            fpSpread1.Sheets[0].Columns[3 + col_idx].CellType = num2;
                        }
                        else
                        {
                        ...
                        fpSpread1.Sheets[0].Columns[3 + col_idx].Width = 90;
                ...
                for (int row = 0; row < fpSpread1.Sheets[0].Rows.Count; row++)
                {
                    ...
                    for (int col = 3; col < fpSpread1.Sheets[0].Columns.Count; col++)
                    {
        ...
        private void CSB011_Activated(object sender, System.EventArgs e)
        {
            if (form_act_chk == false) dtpDtFr.Focus();
        }

        private void CSB011_Deactivate(object sender, System.EventArgs e)
        {
            form_act_chk = true;
        }
        #endregion

...
namespace QM.QMA001
{
    public partial class QMA001 : UIForm.FPCOMM1
    {
        #region 변수선언
        bool Linked = false;
        string strPlantCd = "";
        ...
        public QMA001(string param1, string param2, string param3, string param4)
        {
            //
        ...
        #region Form Load시
        private void QMA001_Load(object sender, System.EventArgs e)
        {
            //필수체크
            ...
            try
            {
                ...
                else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "불량수량"))
                {
					...
					if (string.Compare(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "Lot 추적")].Text, "True", true) == 0 &&
						Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "불량수량")].Value) != 0 &&
                        fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "검사분류")].Text != "P")  //2014-12-29 공정검사는 제외한다. BY KCJ
					{
						MessageBox.Show("Lot 추적 품목은 심플검사에서\r\n불량처리를 할 수 없습니다.", SystemBase.Base.MessageRtn("Z0003"), MessageBoxButtons.OK, MessageBoxIcon.Warning);

    ...
    public partial class PBA119 : UIForm.FPCOMM1
    {
        ...
        private void btnJob_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                UIForm.FPPOPUP pu = new UIForm.FPPOPUP("P00025", strQuery, strWhere, strSearch, new int[] { 0, 1 }, "공정코드 조회");
                pu.Width = 500;
                pu.ShowDialog();
                if (pu.DialogResult == DialogResult.OK)
                {
        ...
        protected override void ExcelExec()
        {
            try
            {
                ...
                else if (rdoBomOut.Checked == true) ExcelCreate("O");
                else ExcelCreate("");
        ...
        private void ExcelCreate(string major_yn)
        {
            ...
            if (major_yn == "Y")
                ...
                dlg.FileName = this.Text.ToString().Replace(@"/", "_") + "_보조라우팅포함(외주).xls";
            else
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    string Query = "Select cd_nm From B_COMM_CODE(Nolock) Where MAJOR_CD = 'T001' AND COMP_CODE = '"+ SystemBase.Base.gstrCOMCD +"' ORDER BY MINOR_CD ";
                        ...
                        if (fpSpread1.Sheets[0].Cells[rowNo, col_idx].Value.ToString() != "30")
                        {
                            ...
                            if (dt1.Rows.Count > 0)
                            {
                                for (int j = 0; j < dt1.Rows.Count; j++)
                                {
                                    ...
                                    Query2 += ", @pMAJOR_FLG = '" + major_yn + "'";
                                    Query2 += " , @pJOB_CD  = '" + txtJobCd.Text.Trim() + "'";

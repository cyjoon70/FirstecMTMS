#region 작성정보
/*********************************************************************/
// 단위업무명 : 프로젝트별 차수별 품목별 공수현황
// 작 성 자 : 김 현 근
// 작 성 일 : 2013-04-23
// 작성내용 : 프로젝트별 차수별 품목별 공수현황
// 수 정 일 :
// 수 정 자 :
// 수정내용 :
// 비    고 :
...
namespace EM.EMR007
{
    public partial class EMR007 : UIForm.FPCOMM2
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void EMR007_Load(object sender, System.EventArgs e)
        {
            ...
            if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false) == true)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead == "U")
                        {
                            string strSql = " usp_EMR007 'U1' ";
                            strSql += ", @pPROJECT_NO = '" + strProjNo + "'";
                            strSql += ", @pITEM_CD = '" + strItemCd + "'";
                            strSql += ", @pPROJECT_SEQ = '" + strProjSeq + "'";

                            strSql += ", @pYYYYMM = '" + fpSpread1.Sheets[0].Cells[i, 1].Text.Replace("-", "") + "'";

                            if (fpSpread1.Sheets[0].Cells[i, 5].Text.Trim() != "")
                                strSql += ", @pEV_RATE  = '" + fpSpread1.Sheets[0].Cells[i, 5].Value + "'";
                            else
                                strSql += ", @pEV_RATE  = 0 ";


                            if (fpSpread1.Sheets[0].Cells[i, 6].Text.Trim() != "")
                                strSql += ", @pCOST_EV_T  = '" + fpSpread1.Sheets[0].Cells[i, 6].Value + "'";
                            else
                                strSql += ", @pCOST_EV_T  = 0 ";

                            strSql += ", @pREMARK = '" + fpSpread1.Sheets[0].Cells[i, 7].Text.Trim() + "'";
        ...
        protected override void fpSpread1_ChangeEvent(int Row, int Col)
        {
            try
            {
                ...
                if (Col == 5 && fpSpread1.Sheets[0].Cells[Row, 5].Text != "")
                {
                    decimal rt = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, 5].Value.ToString());
                    decimal pv = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, 2].Value.ToString());
                    fpSpread1.Sheets[0].Cells[Row, 6].Value = rt / 100 * pv;
        ...
        private void EMR007_Activated(object sender, System.EventArgs e)
        {
            if (form_act_chk == false) txtProjectNo.Focus();
        }

        private void EMR007_Deactivate(object sender, System.EventArgs e)
        {
            form_act_chk = true;
        }

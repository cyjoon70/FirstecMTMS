...
namespace MV.MIV001
{
    public partial class MIV001P2 : UIForm.FPCOMM1
    {
        #region 변수선언
        string strBtn = "N";
        string returnVal;
        ...
        #region 폼로드 이벤트
        private void MIV001P2_Load(object sender, System.EventArgs e)
        {
            ...
            dtpMvmtDtTo.Text = SystemBase.Base.ServerTime("YYMMDD").ToString().Substring(0,10);
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                string strQuery = " usp_MIV001  @pTYPE = 'P3'";
        ...
        private void btnOk_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                RtnStr(fpSpread1.Sheets[0].Cells[TmpRow, SystemBase.Base.GridHeadIndex(GHIdx1, "입고번호")].Text);
        ...
        {
            RtnStr(fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "입고번호")].Text);
        ...
        private void txtIoType_TextChanged(object sender, System.EventArgs e)
        {
            try
            {
                if (strBtn == "N")
                {
                    if (txtIoType.Text != "")
                    {
                        txtIoTypeNm.Value = SystemBase.Base.CodeName("IO_TYPE", "IO_TYPE_NM", "M_MVMT_TYPE", txtIoType.Text, " AND RCPT_YN	= 'Y' AND IM_YN  = 'N' AND CO_CD = '" + SystemBase.Base.gstrCOMCD + "' ");
                    }
                    else
                    {
                        txtIoTypeNm.Value = "";

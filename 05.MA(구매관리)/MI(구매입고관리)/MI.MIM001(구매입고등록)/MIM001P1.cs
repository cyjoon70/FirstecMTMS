...
{ 
    public partial class MIM001P1 : UIForm.FPCOMM1
    {
            ...
            try
            {
                ...
                for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                {
                    if (fpSpread1.Sheets[0].Cells[i, col_sel].Text == "True")
                    {
						...
						spd.Sheets[0].Cells[j, 21].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "입고창고")].Text;
						spd.Sheets[0].Cells[j, 22].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "창고위치")].Value.ToString();
						spd.Sheets[0].Cells[j, 24].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "창고위치")].Text;
        ...
        public void RtnStr(int Row)
        {
            ...
            // 2017.03.02. hma 수정(Start): 입고대상참조 팝업에 변경납기일자 항목을 추가하여 컬럼 위치를 1 증가함. 26=>27로.
            //for (int i = 26; i < fpSpread1.Sheets[0].Columns.Count - 3; i++)
            //{
            //    returnVal[i - 26] = fpSpread1.Sheets[0].Cells[Row, i + 1].Text.ToString();
            for (int i = 27; i < fpSpread1.Sheets[0].Columns.Count - 3; i++)
            {
                returnVal[i - 27] = fpSpread1.Sheets[0].Cells[Row, i + 1].Text.ToString();
            }
            // 2017.03.02. hma 수정(End)
        }
        #endregion

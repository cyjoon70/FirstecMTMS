...
namespace QB.QBA023
{
    public partial class QBA023 : UIForm.FPCOMM1
    {
        #region 생성자
        public QBA023()
        {
        ...
        #region Form Load 시
        private void QBA023_Load(object sender, System.EventArgs e)
        {
            ///필수 항목 체크
            ...
            G1Etc[SystemBase.Base.GridHeadIndex(GHIdx1, "재고처리유형")] = SystemBase.ComboMake.ComboOnGrid("usp_B_COMMON @pType='COMM', @pCODE = 'Q002', @pLANG_CD = '" + SystemBase.Base.gstrLangCd.ToString() + "' , @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'", 0); //재고처리유형 
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                string strQuery = " usp_QBA023 @pTYPE = 'S1'";
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead.Length > 0)
                        {
                            ...
                            string strQuery = " usp_QBA023 @pTYPE = '" + strGbn + "'";
                            strQuery += ", @pINSP_CLASS_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "검사분류코드")].Value + "'";
                            strQuery += ", @pDISPOSAL_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부적합처리코드")].Text + "'";
                            strQuery += ", @pDISPOSAL_NM = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부적합처리명")].Text + "'";
                            strQuery += ", @pSTOCK_TYPE = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "재고처리유형")].Value + "'";

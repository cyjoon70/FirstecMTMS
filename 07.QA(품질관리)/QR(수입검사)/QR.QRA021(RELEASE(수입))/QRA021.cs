...
namespace QR.QRA021
{
    public partial class QRA021 : UIForm.FPCOMM1
    {
        ...
        public QRA021(string param1, string param2, string param3, string param4)
        {
            strInspReqNo = param1;
            strPlantCd = param2;
            strInspReqDt = param3;
        ...
        #region Form Load시
        private void QRA021_Load(object sender, System.EventArgs e)
        {
            //필수체크
            ...
            lnkJump1.Text = "검사항목등록";  //화면에 보여지는 링크명
            strJumpFileName1 = "QR.QRA001.QRA001"; //호출할 화면명
        }
        #endregion
        ...
        private void Grid_Search()
        {
            ...
            try
            {
                string strQuery = " usp_QRA021  @pTYPE = 'S1'";
                ...
                if (fpSpread1.Sheets[0].Rows.Count > 0)
                {
                    ...
                    fpSpread1.Focus();
                    fpSpread1.ActiveSheet.SetActiveCell(SearchRow, 1); //Row Focus		
                    fpSpread1.ShowRow(0, SearchRow, FarPoint.Win.Spread.VerticalPosition.Center); //Focus Row 보기

					SetLotDefectInfo(fpSpread1.Sheets[0].ActiveRowIndex);
                }
                else
                {
                    SystemBase.Validation.GroupBox_Reset(groupBox2);
		...
		#region lot별 불량 정보 설정
		private void SetLotDefectInfo(int row)
		{
			rtxLotInfoM.Value = "";
			string strQuery = string.Empty;
			
			strQuery = " usp_T_IN_INFO_CUDR  @pTYPE = 'S2'";
			strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
			strQuery += ", @pPLANT_CD = '" + cboSPlantCd.SelectedValue + "' ";
			strQuery += ", @pINSP_REQ_NO = '" + fpSpread1.Sheets[0].Cells[row, SystemBase.Base.GridHeadIndex(GHIdx1, "검사의뢰번호")].Text + "'";

			DataTable dtLotM = SystemBase.DbOpen.NoTranDataTable(strQuery);

			if (dtLotM.Rows.Count > 0)
			{
				for (int i = 0; i <= dtLotM.Rows.Count - 1; i++)
				{
					rtxLotInfoM.Value += dtLotM.Rows[i]["LOT_NO"].ToString() + " / " + dtLotM.Rows[i]["DEFECT_QTY"].ToString() + "\r\n";
				}
			}
		}
		#endregion
		...
		private void SubSearch(int iRow)
        {
            ...
            txtBpCd.Value = fpSpread1.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx1, "공급처")].Text;
                        ...
                        if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", true))
                        {
                            ...
                            for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                            {
                                ...
                                if (strHead.Length > 0)
                                {
                                    ...
                                    string strSql = " usp_QRA021 '" + strGbn + "'";
        ...
        private void butInspResult_Click(object sender, System.EventArgs e)
        {
            if (txtInspReqNo.Text != "")
            {
                ...
                try
                {
                    ...
                    if (txUnitytInspReqNo.Text.ToString() != "")
                    {
                        strQuery += ", @pINSP_REQ_NO = '" + txUnitytInspReqNo.Text + "' ";
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++) //내용입력
                        {
                            ...
                            else if (strInspItemCd != "")
                            {
                                ...
                                if (iNextRow == NextPage)
                                {
                                    iRow = iNextRow;
                                    iPage++;
                                    excel.SetCell(iRow, 24, dt.Rows[i]["TPAGE"].ToString() + " 매중 " + iPage.ToString() + " 매");
		...
		#region 증빙조회 후 RowState 조정
		private void fpSpread1_ButtonClicked(object sender, FarPoint.Win.Spread.EditorNotifyEventArgs e)
		{
			if (e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "증빙"))
			{

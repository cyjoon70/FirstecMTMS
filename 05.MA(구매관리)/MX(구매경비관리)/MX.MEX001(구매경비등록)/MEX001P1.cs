...
namespace MX.MEX001
{
    public partial class MEX001P1 : UIForm.FPCOMM2_2
    {
        ...
        string stepsNm = "";
        bool tempID = false;
        string strMinusSlipNo = "";     // 2022.01.28. hma 추가: 반제전표번호
        string strMinusConfirmYn = "";  // 2022.01.28. hma 추가: 반제전표승인여부
        string strMinusGwStatus = "";   // 2022.01.28. hma 추가: 반제전표결재상태
        #endregion
        ...
        public MEX001P1(string ExpNo, string ExpStep, string ExpStepNm, string Is, decimal amt_loc1, decimal xrate1, string cfm, decimal amt1, 
                        string MSlipNo, string MConfirmYn, string MGwStatus,            // 2022.01.28. hma 추가
                        bool div)
        {
            InitializeComponent();
            ...
            strMinusSlipNo = MSlipNo;           // 2022.01.28. hma 추가: 반제전표번호
            strMinusConfirmYn = MConfirmYn;     // 2022.01.28. hma 추가: 반제전표 승인여부
            strMinusGwStatus = MGwStatus;       // 2022.01.28. hma 추가: 반제전표 결재상태
        ...
        private void MEX001P1_Load(object sender, System.EventArgs e)
        {
            ...
            // 2022.01.28. hma 수정(Start): 미확정상태이지만 반제전표 결재상태가 승인이 아닌 경우에도 툴바 버튼들 비활성화 되게.
            //if (cfmYN == "True")
            if ((cfmYN == "True")
                // 2022.04.27. hma 수정(Start): 미확정상태이면서 반제전표번호가 존재하고 반제승인이 안되었거나 반제전표 승인상태가 아닌경우
                //|| (strMinusSlipNo == "" || (strMinusSlipNo != "" && (strMinusConfirmYn != "Y" || strMinusGwStatus != "APPR"))) )
                || (cfmYN == "False" && (strMinusSlipNo != "" && (strMinusConfirmYn != "Y" || strMinusGwStatus != "APPR"))))

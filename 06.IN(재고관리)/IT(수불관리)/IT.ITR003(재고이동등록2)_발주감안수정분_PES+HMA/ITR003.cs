...
namespace IT.ITR003
{
    public partial class ITR003 : UIForm.FPCOMM2
    {
        ...
        private void btnREQ_PO_REF_Click(object sender, EventArgs e)
        {
            ...
            // 2017.09.11. hma 추가(Start)
            if (txtMoveType.Text.Trim() != "T13")
            {
                MessageBox.Show("수불유형이 재고이동(발주감안) 경우에만 발주감안참조 하실 수 있습니다.", SystemBase.Base.MessageRtn("Z0003"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtMoveType.Focus();
                return;
            }
            // 2017.09.11. hma 추가(End)

            // 2017.09.11. hma 수정: 필수 항목 체크하지 않도록 주석 처리함.
            //if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox2) && SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox4))
            //{
            try
            {
                    ...
                    if (pu.DialogResult == DialogResult.OK)
                    {
                        // 2017.09.11. hma 추가(Start): 발주감안 이동 팝업창 결과 체크
                        if (pu.ReturnVal[0] == "Y")
                        {
                            txtProjectNo2.ReadOnly = false;
                            txtProjectSeq2.ReadOnly = false;
                            txtProjectNo2.Text = pu.ReturnVal[1];
                            txtProjectSeq2.Text = pu.ReturnVal[2];
                            txtProjectNo2.ReadOnly = true;
                            txtProjectSeq2.ReadOnly = true;
                            txtSlCd2.Text = "W03";
                        }
                        // 2017.09.11. hma 추가(End)

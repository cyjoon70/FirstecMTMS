...
namespace AH.ACH005
{
    public partial class ACH005 : UIForm.FPCOMM2
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void ACH005_Load(object sender, System.EventArgs e)
        {
            //그룹박스 필수체크
        ...
        private void Control_Setting()
        {
            ...
            dtxtExchRate.Tag = ";2;;;";
            cboBizAreaCd.SelectedValue = "";
            cboVatType.SelectedValue = "";
            dtxtVatRate.Value = "";
        ...
        private void Search(bool bSeachFlag)
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_ACH005  @pTYPE = 'S1'";
        ...
        private void SubSearch(string strCode)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows[0]["CHANGE_DIV"].ToString() == "03") optChangeDiv3.Checked = true;
                    else optChangeDiv4.Checked = true;
                    ...
                    txtSlipConfirmYn.Value = dt.Rows[0]["CFM_SLIP_CONFIRM_YN"].ToString();              // 전표번호 확정여부
                    ...
                    else
                    {
                        ...
                        // 미확정 상태이지만 반제전표번호가 없거나, 반제전표번호가 있으면서 반제전표결재상태가 승인이 아닐 경우 항목들 Lock.
                        if ((txtMSlipNo.Text == "") ||
                            (txtMSlipNo.Text != "" &&
        ...
        private void Set_Lock_yn(string strLock)
        {
                ...
                || (strSlipConfirmYn != "Y" && strMinusSlipNo != "" && strMinusGwStatus != "APPR")) // 2022.01.28. hma 추가: 미확정 상태이지만 반제전표를 생성해서 승인상태가 아닌 경우도 비활성화
            {
                ...
                {
                    UIForm.FPMake.grdReMake(fpSpread1, i,
                            SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액") + "|2"
                            + "#" + SystemBase.Base.GridHeadIndex(GHIdx1, "변동수량") + "|2"    // 2022.01.28. hma 추가
                            + "#" + SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액") + "|2"
            ...
            else
            {
                ...
                {
                    UIForm.FPMake.grdReMake(fpSpread1, i,
                            SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액") + "|1"
                            + "#" + SystemBase.Base.GridHeadIndex(GHIdx1, "변동수량") + "|1"    // 2022.01.28. hma 추가
                            + "#" + SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액") + "|1"
                    ...
                    if (fpSpread1.Sheets[0].Rows.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (NewFlg != 0)
                            {
                                ...
                                string strSql = " usp_ACH005 '" + strMstType + "'";
                                strSql += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                                ...
                                if(optChangeDiv3.Checked == true) strSql += ", @pCHANGE_DIV = '03' ";
                                else if(optChangeDiv4.Checked == true) strSql += ", @pCHANGE_DIV = '04' ";
                            ...
                            if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false) == true)
                            {
                                ...
                                if (DelCheck() == false)
                                {
                                    ...
                                    if (dsMsg == DialogResult.Yes)
                                    {
                                        try
                                        {
                                            string strDelSql = " usp_ACH005  'D1'";
                                ...
                                for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                                {
                                    ...
                                    if (strHead.Length > 0)
                                    {
                                        ...
                                        string strSql = " usp_ACH005 '" + strGbn + "'";
                                        strSql += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                                        strSql += ", @pCHANGE_NO = '" + strAutoChangeNo + "' ";
                                        strSql += ", @pCHANGE_DT = '" + dtpChangeDt.Text + "' ";
                                        strSql += ", @pASSET_NO = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "자산번호")].Value + "' ";
                                        strSql += ", @pCHANGE_QTY = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동수량")].Text.Replace(",","") + "' ";
                                        strSql += ", @pCHANGE_AMT = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Text.Replace(",", "") + "' ";
                                        strSql += ", @pCHANGE_AMT_LOC = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Text.Replace(",", "") + "' ";
                                        strSql += ", @pVAT_AMT = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부가세")].Text.Replace(",", "") + "' ";
                                        strSql += ", @pVAT_AMT_LOC = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부가세자국")].Text.Replace(",", "") + "' ";
        ...
        protected override void fpSpread1_ChangeEvent(int Row, int Column)
        {
            ...
            {
                string Query = " usp_ACH005 @pTYPE = 'C1', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "', @pASSET_NO = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "자산번호")].Text + "' ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "재고수량")].Value = dt.Rows[0]["ASSET_QTY"].ToString();
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동수량")].Value = dt.Rows[0]["ASSET_QTY"].ToString();

                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "취득금액")].Value = dt.Rows[0]["ASSET_AMT"].ToString();
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "감가상각누계액")].Value = dt.Rows[0]["DEPR_SUM_AMT"].ToString();
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "상각잔액")].Value = dt.Rows[0]["DEPR_BAL_AMT"].ToString();

                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Value = dt.Rows[0]["DEPR_BAL_AMT"].ToString();
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "부가세")].Value
                            = Math.Floor(Convert.ToDouble(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Value) * (dblVat_Rate * 0.01));
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Value
                            = Math.Floor(Convert.ToDouble(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Value) * dblExch_Rate);
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "부가세자국")].Value
                            = Math.Floor(Convert.ToDouble(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Value) * dblExch_Rate * (dblVat_Rate * 0.01));
                    SumAmt();
                }
                else
                {
                    ...
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "재고수량")].Text = "";
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동수량")].Text = "";
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "취득금액")].Text = "";
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "감가상각누계액")].Text = "";
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "상각잔액")].Text = "";

                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Text = "";
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Text = "";
            ...
            else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액") || Column == SystemBase.Base.GridHeadIndex(GHIdx1, "부가세"))
            {
                double dblExch_Rate = 0;
                if(dtxtExchRate.Text != "") dblExch_Rate = Convert.ToDouble(dtxtExchRate.Text.Replace(",", ""));
                double dblVat_Rate = 0;
                if(dtxtVatRate.Text != "") dblVat_Rate = Convert.ToDouble(dtxtVatRate.Text.Replace(",",""));
                //변동금액
                if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액"))
                {
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "부가세")].Value
                            = Math.Floor(Convert.ToDouble(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Value) * (dblVat_Rate * 0.01));
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Value
            ...
            else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액") || Column == SystemBase.Base.GridHeadIndex(GHIdx1, "부가세자국"))
            {
                SumAmt();
            }
        }
        ...
        private void SumAmt()
        {
            ...
            for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
            {
                if (fpSpread1.Sheets[0].RowHeader.Cells[i, 0].Text != "D")
                {
                    if(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Text != "")
                        dAmt += Convert.ToDouble(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동금액")].Value);
                    if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Text != "") 
                        dAmtLoc += Convert.ToDouble(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Value);
            ...
            try
            {
                if (NewFlg != 0)
                {
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "변동자국금액")].Value
        ...
        //변동구분
        private void optChangeDiv3_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (optChangeDiv3.Checked == true)
                {
                    cboVatType.Tag = "부가세유형;;;";
                    cboBizAreaCd.Tag = "세금신고사업장;;;";
                    txtAcctCd.Tag = "변동계정코드;1;;";
                    btnAcct.Tag = ";;;";
                    dtxtVatRate.Tag = "부가세율;;;";
                }
                else
                {
                    cboVatType.SelectedValue = "";
                    ...
                    cboVatType.Tag = "부가세유형;2;;";
                    cboBizAreaCd.Tag = "세금신고사업장;2;;";
                    txtAcctCd.Tag = "변동계정코드;2;;";
                    btnAcct.Tag = ";2;;";
                    dtxtVatRate.Tag = "부가세율;2;;";
                }
                SystemBase.Validation.GroupBox_Setting(groupBox2);
            }
            catch (Exception f)

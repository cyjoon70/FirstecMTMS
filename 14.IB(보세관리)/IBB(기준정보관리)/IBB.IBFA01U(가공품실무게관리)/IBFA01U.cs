...
namespace IBB.IBFA01U
{
    public partial class IBFA01U : UIForm.FPCOMM1
    {
        ...
        #region Form Load 시
        private void IBFA01U_Load(object sender, System.EventArgs e)
        {
        ...
        protected override void SearchExec()
        {
             ...
             try
             {
                 ...
                 {
                     string strQuery = " usp_IBFA01U  'S1'";
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead.Length > 0)
                        {
                            ...
                            string strQuery = " usp_IBFA01U '" + strGbn + "'";
                            strQuery = strQuery + ", @pITEM_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "품목")].Text + "'";
                            strQuery = strQuery + ", @pREAL_WEIGHT = '" + Convert.ToDouble(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "가공품 실무게")].Text) + "'";
                            strQuery = strQuery + ", @pBASIC_UNIT = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "단위")].Text + "'";
                            strQuery = strQuery + ", @pMATTER = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "형태")].Text + "'";
        ...
        protected override void fpButtonClick(int Row, int Column)
        {
            if (fpSpread1.Sheets[0].RowHeader.Cells[Row, 0].Text == "I")
            {
                ...
                //창고
                if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "단위_2"))
                {
                    try
                    {
                        string strQuery = " Nusp_BF_Comm 'BF05', @pCO_CD = '"+ SystemBase.Base.gstrCOMCD.ToString() +"' ";
                        ...
                        if (pu.DialogResult == DialogResult.OK)
                        {
							...
							if(fpSpread1.Sheets[0].RowHeader.Cells[Row,0].Text != "I") fpSpread1.Sheets[0].RowHeader.Cells[Row,0].Text = "U";
                    ...
                    {
                        SystemBase.Loggers.Log(this.Name, f.ToString());
                        MessageBox.Show(SystemBase.Base.MessageRtn("B0050", "단위 팝업"), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "형태_2"))
                {
                    try
                    {
                        string strQuery = " Nusp_BF_Comm 'BF06', @pCO_CD = '" + SystemBase.Base.gstrCOMCD.ToString() + "'  ";

                        string[] strWhere = new string[] { "@pValue" };
                        string[] strSearch = new string[] { fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "형태")].Text };
                        ...
                        if (pu.DialogResult == DialogResult.OK)
                        {
                            ...
                            if (fpSpread1.Sheets[0].RowHeader.Cells[Row, 0].Text != "I") fpSpread1.Sheets[0].RowHeader.Cells[Row, 0].Text = "U";
                            fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "형태")].Text = Msgs[0].ToString();
                            fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "형태명")].Text = Msgs[1].ToString();
                    ...
                    {
                        SystemBase.Loggers.Log(this.Name, f.ToString());
                        MessageBox.Show(SystemBase.Base.MessageRtn("B0050", "형태 팝업"), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }
        ...
        protected override void fpSpread1_ChangeEvent(int Row, int Col)
        {
            try
            {
                ...
                if (Col == SystemBase.Base.GridHeadIndex(GHIdx1, "형태"))
                {
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "형태명")].Text = SystemBase.Base.CodeName("MINOR_CD", "CD_NM", "B_COMM_CODE", fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "형태")].Text, " AND MAJOR_CD = 'BF001' AND LANG_CD = '" + SystemBase.Base.gstrLangCd + "' AND COMP_CODE='" + SystemBase.Base.gstrCOMCD + "'");
                }
            }
            catch (Exception f)
            {
        ...
        private void IBFA01U_Activated(object sender, System.EventArgs e)
        {
            if (chk == false)
            {
                txtItemCd.Focus();
        ...
        private void IBFA01U_Deactivate(object sender, System.EventArgs e)
        {
            chk = true;
        }
        #endregion

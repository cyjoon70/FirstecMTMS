...
namespace PC.PCD001
{
    public partial class PCD001 : UIForm.FPCOMM1
    {
        #region 생성자
        public PCD001()
        {
        ...
        protected override void SearchExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1) == true)
            {
                try
                {
                    string strQuery = "usp_PCD001 @pTYPE = 'S1'";
        ...
        private void butReqRef_Click(object sender, EventArgs e)
        {
            if (fpSpread1.ActiveSheet.GetSelection(0) != null)
            {
                try
                {
                    ...
                    PCD001P1 myForm = new PCD001P1(strPoNo, strProjectNo, strProjectSeq, strItemCd, strItemNm);
                    myForm.ShowDialog();
                }
                catch (Exception f)
                {
        ...
        private void butMrpRef_Click(object sender, EventArgs e)
        {
            if (fpSpread1.ActiveSheet.GetSelection(0) != null)
            {
                try
                {
                    ...
                    PCD001P2 myForm = new PCD001P2(strPoNo, strProjectNo, strProjectSeq, strItemCd, strItemNm);
                    myForm.ShowDialog();
                }
                catch (Exception f)
                {

...
namespace AE.ACE007
{
    public partial class ACE007 : UIForm.FPCOMM1 
    {
        public ACE007()
        {
            InitializeComponent();
        ...
        #region Form Load 시
        private void ACE007_Load(object sender, System.EventArgs e)
        {
        ...
        private void btnTypeCd_Click(object sender, EventArgs e)
        {
            try
            {
                try
                {
                    string strQuery = " usp_B_COMMON @pType='COMM_POP', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "', @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "', @pSPEC1 = 'A120', @pSPEC2 = 'PL' ";
        ...
        private void txtTypeCd_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txtTypeNm.Value = SystemBase.Base.CodeName("MINOR_CD", "CD_NM", "B_COMM_CODE", txtTypeCd.Text, " AND LANG_CD = '" + SystemBase.Base.gstrLangCd + "' AND REL_CD1 = 'PL' AND COMP_CODE = '" + SystemBase.Base.gstrCOMCD + "'");
            }
            catch (Exception f)
            {
                MessageBox.Show(f.ToString(), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        protected override void PrintExec()
        {
            try
            {
                if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
                {
                    ...
                    string RptName = SystemBase.Base.ProgramWhere + @"\Report\ACE007.rpt";    // 레포트경로+레포트명

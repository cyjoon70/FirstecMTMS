...
namespace SS.SSA002
{
    public partial class SSA002 : UIForm.FPCOMM2
    {
        ...
        private void SSA002_Load(object sender, System.EventArgs e)
        {
            ...
            SystemBase.ComboMake.C1Combo(cboCSlipGwStatus, "usp_B_COMMON @pType='COMM', @pCODE = 'B094', @pLANG_CD = '" + SystemBase.Base.gstrLangCd.ToString() + "', @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'", 0); // 2022.01.28. hma 추가: 그룹웨어상태
            SystemBase.ComboMake.C1Combo(cboMSlipGwStatus, "usp_B_COMMON @pType='COMM', @pCODE = 'B094', @pLANG_CD = '" + SystemBase.Base.gstrLangCd.ToString() + "', @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'", 0); // 2022.01.28. hma 추가: 그룹웨어상태
        ...
        private void SubSearch(string Code)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    // 2022.01.28. hma 추가(Start): 결재상태 및 반제전표번호, 반제승인 추가
                    ...
                    if (ConfirmChk == true)
                    {
                        ...
                        else
                        {
                            ...
                            // 2022.01.28. hma 추가(Start): 확정상태이면서 결재상태가 상신대기/반려/승인 상태이면 확정취소 버튼 활성화되게.
                    ...
                    {
                        // 2022.01.28. hma 추가(Start): 확정/반제 전표 체크 & 반제 승인 아닌 경우 비활성화 처리
                        ...
                        // 전표승인여부가 Y가 아니고 반제전표번호가 존재하며 반제전표결재상태가 승인이면
                        ...
                        // 2022.01.28. hma 수정(Start): 미확정상태이지만 반제전표 결재상태가 상신대기/반려 상태일때만 확정취소 버튼 활성화되게.
                        ...
                        if ((txtMSlipNo.Text == "") ||
                            ...
                            btnBnOk.Enabled = false;
                        // 2022.01.28. hma 수정(End)

                        // 2022.01.28. hma 수정(Start): 미확정건이지만 반제전표가 생성된 경우에는 확정취소 버튼 비활성화 처리.
                        ...
                        // 2022.01.28. hma 추가(Start): 반제전표 결재상태에 따라 반제취소 버튼 활성화 처리. 반제전표 결재상태가 상신대기, 반려이면 활성화.
        ...
        private void Confirm(string strConfirmYn)
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox2))
            {
                ...
                try
                {
                    ...
                    strSql += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                    strSql += ", @pUP_ID = '" + SystemBase.Base.gstrUserID + "'";       // 2022.02.14. hma 추가: 반제전표 생성을 위해 추가.

...
namespace PA.PBA110
{
    public partial class PBA110 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void PBA110_Load(object sender, System.EventArgs e)
        {
            //필수체크
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                        {
                            ...
                            if (strHead.Length > 0)
                            {
                                ...
                                string strSql = " usp_PBA110 '" + strGbn + "'";
                                ...
                                strSql += ", @pAVAILABLE_TIME = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "O/T자원능력")].Value + "'";
                                ...
                                if (strHead == "I") // 등록
                                    strSql += ", @pSCH_ID = '" + cboSchId.SelectedValue + "'";
                                else
        ...
        {
            if (e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "O/T자원능력") || e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "대수") || e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "효율(%)"))
            {
                decimal AVAILABLE_TIME = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "O/T자원능력")].Value);

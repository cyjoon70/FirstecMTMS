#region 작성정보
/*********************************************************************/
// 단위업무명 : 프로젝트별 차수별 품목별 성과(EV)현황
// 작 성 자 : 김 현 근
// 작 성 일 : 2013-04-23
// 작성내용 : 프로젝트별 차수별 품목별 성과(EV)현황
// 수 정 일 :
// 수 정 자 :
// 수정내용 :
// 비    고 :
...
namespace EM.EMR013
{
    public partial class EMR013 : UIForm.FPCOMM2
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void EMR013_Load(object sender, System.EventArgs e)
        {
        ...
        private void Set_Merge()
        {
            ...
            for (int i = 0; i < fpSpread1.Sheets[0].RowCount; i++)
            {
                ...
                if (i == 0)
                {
                    temp_ym = fpSpread1.Sheets[0].Cells[i, 1].Text;
                    temp_class = fpSpread1.Sheets[0].Cells[i, 2].Text;
                    temp_element = fpSpread1.Sheets[0].Cells[i, 3].Text;
                    first_row_idx1 = i;
                    first_row_idx2 = i;
                }
                else
                {
                    if (temp_ym == fpSpread1.Sheets[0].Cells[i, 1].Text)
                    {
                        rowspan1++;
                    }
                    else
                    {
                        fpSpread1.Sheets[0].Cells[first_row_idx1, 4].RowSpan = rowspan1;
                        fpSpread1.Sheets[0].Cells[first_row_idx1, 4].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                    ...
                    if (temp_class == fpSpread1.Sheets[0].Cells[i, 2].Text)
                    {
                        rowspan2++;
                    }
                    else
                    {
                        fpSpread1.Sheets[0].Cells[first_row_idx2, 5].RowSpan = rowspan2;
                        fpSpread1.Sheets[0].Cells[first_row_idx2, 5].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                    ...
                    temp_element = fpSpread1.Sheets[0].Cells[i, 3].Text;
                    if (temp_class == temp_element) fpSpread1.Sheets[0].Cells[i, 5].ColumnSpan = 2;

                    //소계, 제조원가, 총원가, 계산가격 readonly
                    if (temp_element == "A99" || temp_element == "B99" || temp_element == "C99" || temp_class == "D" || temp_class == "F" || temp_class == "Z")
                    {
                        for (int h = 7; h < fpSpread1.Sheets[0].ColumnCount; h++)
                            UIForm.FPMake.grdReMake(fpSpread1, i, h + "|3");
                    }
        ...
        private void fpSpread1_Summary()
        {
            try
            {
                ...
                fpSpread1.Sheets[0].Rows[fpSpread1.Sheets[0].Rows.Count - 1].Visible = false; //잠시추가 ㅋ
            }
            catch (Exception f)
            {
                SystemBase.Loggers.Log(this.Name, f.ToString());
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead == "U")
                        {
                            ...
                            strSql += ", @pCOST_CLASS = '" + fpSpread1.Sheets[0].Cells[i, 2].Text + "'";
                            strSql += ", @pCOST_ELEMENT = '" + fpSpread1.Sheets[0].Cells[i, 3].Text + "'";

                            if (fpSpread1.Sheets[0].Cells[i, 8].Text.Trim() != "")
                                strSql += ", @pCOST_AV_T  = '" + fpSpread1.Sheets[0].Cells[i, 8].Value + "'";
                            else
                                strSql += ", @pCOST_AV_T  = 0 ";

                            if (fpSpread1.Sheets[0].Cells[i, 9].Text.Trim() != "")
                                strSql += ", @pEV_RATE  = '" + fpSpread1.Sheets[0].Cells[i, 9].Value + "'";
                            else
                                strSql += ", @pEV_RATE  = 0 ";

                            if (fpSpread1.Sheets[0].Cells[i, 10].Text.Trim() != "")
                                strSql += ", @pCOST_EV_T  = '" + fpSpread1.Sheets[0].Cells[i, 10].Value + "'";
                            else
                                strSql += ", @pCOST_EV_T  = 0 ";

                            strSql += ", @pREMARK = '" + fpSpread1.Sheets[0].Cells[i, 11].Text.Trim() + "'";
        ...
        protected override void fpSpread1_ChangeEvent(int Row, int Col)
        {
            try
            {
                if (Col == 9 && fpSpread1.Sheets[0].Cells[Row, 9].Text != "" && fpSpread1.Sheets[0].Cells[Row, 7].Text != "")
                {
                    decimal rt = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, 9].Value.ToString());
                    decimal pv = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, 7].Value.ToString());
                    fpSpread1.Sheets[0].Cells[Row, 10].Value = rt / 100 * pv;
        ...
        private void EMR013_Activated(object sender, System.EventArgs e)
        {
            if (form_act_chk == false) txtProjectNo.Focus();
        }

        private void EMR013_Deactivate(object sender, System.EventArgs e)
        {
            form_act_chk = true;
        }

...
namespace TD.TDP003
{
	public partial class TDP003 : UIForm.FPCOMM2
	{
		#region 필드
		/// <summary>문서카테고리 코드</summary>
		const string docCtgCd = "AOR";      // 구매 외주입고
		...
		#region 폼 이벤트 핸들러
		private void TDP003_Load(object sender, System.EventArgs e)
		{
			// 필수체크
			...
			// 그리드초기화
			G1Etc[SystemBase.Base.GridHeadIndex(GHIdx1, "문서종류")] = SystemBase.ComboMake.ComboOnGrid("usp_T_DOC_CODE @pTYPE = 'S1', @pDOC_CTG_CD = '" + docCtgCd + "', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "'", 0);
                ...
                for (int row = 0; row < sheet.RowCount; row++)
                {
                    ...
                    for (int srow = 0; srow < subSheet.Rows.Count; srow++)
                    {
                        ...
                        if (iSlipCnt == 0)
                        {
                            // 키 = "제조오더번호/공정번호/실적순번"
                            query += ", @pDOC_CTG_CD = '" + docCtgCd + "'"
                                    + ", @pATT_KEY = '" + subSheet.Cells[srow, colWorkorderNo].Text + "/" + subSheet.Cells[srow, colProcSeq].Text + "/" + subSheet.Cells[srow, colSeq].Text + "'"
                                    + ", @pATT_KEY1 = '" + subSheet.Cells[srow, colWorkorderNo].Text + "'"
                                    + ", @pATT_KEY2 = '" + subSheet.Cells[srow, colProcSeq].Text + "'"
                                    + ", @pATT_KEY3 = '" + subSheet.Cells[srow, colSeq].Text + "'"
        ...
        private void btnIN_ID_Click(object sender, EventArgs e)
        {
            try
            {
                string strQuery = " usp_M_COMMON 'M014', @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'";
                string[] strWhere = new string[] { "@pCODE", "@pNAME" };
                string[] strSearch = new string[] { txtIN_ID.Text, "" };

                UIForm.FPPOPUP pu = new UIForm.FPPOPUP("P00031", strQuery, strWhere, strSearch, new int[] { 0, 1 }, "등록자 팝업");
                pu.ShowDialog();
        ...
        private void txtIN_ID_TextChanged(object sender, System.EventArgs e)
        {
            try
            {
                txtIN_IDNM.Value = !string.IsNullOrEmpty(txtIN_ID.Text) ? SystemBase.Base.CodeName("USR_ID", "USR_NM", "B_SYS_USER", txtIN_ID.Text, " AND CO_CD ='" + SystemBase.Base.gstrCOMCD + "'") : "";
            }
            catch
            {

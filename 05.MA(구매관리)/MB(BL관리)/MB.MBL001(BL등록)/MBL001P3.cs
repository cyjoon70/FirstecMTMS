...
namespace MB.MBL001
{
    public partial class MBL001P3 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        public MBL001P3(FarPoint.Win.Spread.FpSpread spread)
        {
            InitializeComponent();
            spd = spread;
        }
        ...
        #region 폼로드 이벤트
        private void MBL001P3_Load(object sender, EventArgs e)
        {
            this.Text = "L/C 참조 팝업";
        ...
        private void Grid_Search(bool Msg)
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_MBL001  @pTYPE = 'P4'";
                    strQuery += ", @pLC_DT_FR = '" + dtpLcDtFr.Text + "' ";
                    strQuery += ", @pLC_DT_TO = '" + dtpLcDtTo.Text + "' ";
                ...
                if (fpSpread1.Sheets[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        if (fpSpread1.Sheets[0].Cells[i, col_sel].Text == "True")
                        {
                            ...
                            spd.Sheets[0].Cells[j, 8].Value = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C잔량")].Value;         // 2022.06.09. hma 수정: 7=>8로
                            ...
                            if (Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C잔량")].Value) == 0)
                            {
                                iLcAmt = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C금액")].Value);
                                iLcAmtLoc = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "자국금액")].Value);
                            }
                            else
                            {
                                iLcAmt = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "단가")].Value) * Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C잔량")].Value);
                                iLcAmtLoc = iLcAmt * Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "환율")].Value);
                            }

                            spd.Sheets[0].Cells[j, 10].Value = iLcAmt;       // 2022.06.09. hma 수정: 9=>10으로
                            spd.Sheets[0].Cells[j, 11].Value = iLcAmtLoc;    // 2022.06.09. hma 수정: 10=>11로
                            ...
                            spd.Sheets[0].Cells[j, 27].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C번호")].Text;      // 2022.06.09. hma 수정: 26=>27로
                            spd.Sheets[0].Cells[j, 28].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C순번")].Text;      // 2022.06.09. hma 수정: 27=>28로
                            j++;
                            iRow = i;
                        }
                    }
                    ...
                    if (iRow != -1)
                        RtnStr(fpSpread1.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx1, "L/C번호")].Text);
                }
            }
            catch (Exception f)
            {
                SystemBase.Loggers.Log(this.Name, f.ToString());
                MessageBox.Show(f.Message, SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error); //데이타 조회 중 오류가 발생하였습니다.
            }
            Close();

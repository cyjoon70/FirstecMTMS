...
namespace MB.MBL001
{
    public partial class MBL001P4 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        public MBL001P4(FarPoint.Win.Spread.FpSpread spread)
        {
            InitializeComponent();
            spd = spread;
        }
        ...
        #region 폼로드 이벤트
        private void MBL001P4_Load(object sender, EventArgs e)
        {
            this.Text = "SCM참조팝업";
            ...
            //기타세팅
            dtpSupplyDtFr.Value = null;
            dtpSupplyDtTo.Value = null;
        ...
        private void btnScmMvmtNo_Click(object sender, System.EventArgs e)
        {
            try
            {
                string strQuery = "usp_MBL001 @pTYPE = 'P6', @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "'";
                strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                string[] strWhere = new string[] { "@pSCM_MVMT_NO", "" };
                string[] strSearch = new string[] { txtScmMvmtNo.Text, "" };

                UIForm.FPPOPUP pu = new UIForm.FPPOPUP("P00090", strQuery, strWhere, strSearch, new int[] { 0 }, "SCM번호조회");
                pu.Width = 600;
                pu.ShowDialog();
                ...
                if (pu.DialogResult == DialogResult.OK)
                {
                    ...
                    txtScmMvmtNo.Value = Msgs[0].ToString();
                }
            }
            catch (Exception f)
            {
                SystemBase.Loggers.Log(this.Name, f.ToString());
                MessageBox.Show(SystemBase.Base.MessageRtn("B0050", "SCM번호 팝업"), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void Set_Tag(string div)
        {
            ...
            else if (div == "2")
            {
                ...
                dtpSupplyDtFr.ReadOnly = false;
                dtpSupplyDtTo.ReadOnly = false;

                dtpSupplyDtFr.Tag = ";5;;";
                dtpSupplyDtTo.Tag = ";5;;";
                //기타 세팅
                dtpSupplyDtFr.Value = Convert.ToDateTime(SystemBase.Base.ServerTime("YYMMDD")).AddMonths(-1).ToShortDateString().Substring(0,10);
                dtpSupplyDtTo.Value = SystemBase.Base.ServerTime("YYMMDD").ToString().Substring(0,10);
            }
            else
            {
                ...
                //기타 세팅
                dtpSupplyDtFr.Value = Convert.ToDateTime(SystemBase.Base.ServerTime("YYMMDD")).AddMonths(-1).ToShortDateString().Substring(0,10);
                dtpSupplyDtTo.Value = SystemBase.Base.ServerTime("YYMMDD").Substring(0,10);
            }
            SystemBase.Base.GroupBoxLang(groupBox1); //필수체크
        ...
        private void Grid_Search(bool Msg)
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_MBL001  @pTYPE = 'P5'";
                    strQuery += ", @pSUPPLY_DT_FR = '" + dtpSupplyDtFr.Text + "' ";
                    strQuery += ", @pSUPPLY_DT_TO = '" + dtpSupplyDtTo.Text + "' ";
                ...
                if (fpSpread1.Sheets[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        if (fpSpread1.Sheets[0].Cells[i, col_sel].Text == "True")
                        {
                            ...
                            spd.Sheets[0].Cells[j, 5].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "단위")].Text;
                            ...
                            spd.Sheets[0].Cells[j, 8].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "발주잔량")].Text;          // 2022.06.09. hma 수정: 7=>8로
                            spd.Sheets[0].Cells[j, 9].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "단가")].Text;              // 2022.06.09. hma 수정: 8=>9로
                            ...
                            spd.Sheets[0].Cells[j, 29].Text = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "SCM번호")].Text;      // 2022.06.09. hma 수정: 28=>29로
                            j++;

...
namespace WNDW
{
	public partial class WNDW036 : UIForm.FPCOMM2
	{
		...
		#region Form Load
		private void WNDW036_Load(object sender, EventArgs e)
		{
			...
			// 품목목록 첨부문서표시 관리자 초기화
			attachmentManager = new AttachmentManager(fpSpread2.ActiveSheet, "PUR", null, "첨부문서코드", "필수문서코드")
			{
		...
		protected override void SearchExec()
		{
			...
			try
			{
				...
				string query = "usp_TDQ001 'S2'"
					...
					+ ", @pPLANT_CD = '" + cboPLANT.SelectedValue.ToString() + "'"
					+ ", @pMVMT_NO = '" + txtKEY_NO.Text + "'"
					+ ", @pMVMT_SEQ = '" + strKEY_SEQ + "'";
				...
				attachmentManager.PlantCode = cboPLANT.SelectedValue.ToString();
		...
		private void SearchDocument(int flag)
		{
			...
			if (flag == 2 || string.IsNullOrEmpty(strKEY_SEQ) == false)
			{
				query += ", @pATT_KEY2 = '" + fpSpread2.Sheets[0].Cells[fpSpread2.Sheets[0].ActiveRowIndex, colSubMvmtSeq].Text + "'";

				string barcode = fpSpread2.Sheets[0].Cells[fpSpread2.Sheets[0].ActiveRowIndex, colSubBarCode].Text;     // 입고순번
				if (!string.IsNullOrEmpty(barcode)) query += ", @pATT_KEY3 = '" + barcode + "'";                        // 바코드
			}

...
// 수 정 일 : 2015-08-20
// 수 정 자 : 최 용 준
// 수정내용 : 차수 이동에 따른 lot 재고 처리
//				- 동일 품목, 동일 프로젝트 => 수불만 발생
//				- 동일 품목, 다른 프로젝트 => 수불발생 + LOT 재고 이동
//				- 팝업창이 로딩될 때 해당 lot 재고 정보를 그리드에 출력(fpspread2하고,
//				  선택되는 프로젝트에 따라 해당 lot 정보만 보여주는 방식으로 처리
// 비    고 :
/**********************************************************************************************/
#endregion
...
namespace MR.MRQ003
{
	public partial class MRQ003P3 : UIForm.FPCOMM2
	{
		...
		#region 생성자
		public MRQ003P3(string ReqNo, string ReqSeq, string ItemCd, string ItemNm, decimal ReqQty, decimal RefQty, bool locking, string LotYn, string ProjectNo, string ProjectSeq, string Ref_YN)
        {
            //
		...
		#region Form Load 시
		private void MRQ003P3_Load(object sender, EventArgs e)
		{
			this.Text = "재고참조팝업";
			...
			// 수불 그리드에서 정렬되지 않도록 처리(lot 처리시 중복 처리 방지를 위하여)
			fpSpread1.ContextMenu.MenuItems[1].Enabled = false;
		...
		protected override void SearchExec()
		{
			...
			try
			{
				...
				if (string.Compare(strLotYn, "True", true) == 0)
				{
					...
					// lot 정보 조회 후 데이터가 안보이게 숨김. 이후 fpspread1 행 선택 값의 조건에 따라 데이터 보여줌.
					if (fpSpread2.Sheets[0].Rows.Count > 0)
					{

						int idx2 = SystemBase.Base.GridHeadIndex(GHIdx2, "수불번호");

						for (int i = 0; i < fpSpread2.Sheets[0].Rows.Count; i++)
						{
							if (fpSpread2.Sheets[0].Cells[i, idx2].Text.Trim() != "")
							{
								strTranNo = fpSpread2.Sheets[0].Cells[i, idx2].Text;
								strMovTranNo = fpSpread2.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx2, "이동수불번호")].Text;
								txtTranDuty.Text = fpSpread2.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx2, "수불담당자")].Text;
								txtTranDutyNm.Value = fpSpread2.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx2, "담당자명")].Text;
								break;
							}
						}
						...
						for (int j = 0; j < fpSpread2.Sheets[0].Rows.Count; j++)
						{
							if (locked)
							{
								UIForm.FPMake.grdReMake(fpSpread2, j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택") + "|3#"
										+ SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량") + "|3");
							}
							else if (fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택")].Text == "True")
							{
								UIForm.FPMake.grdReMake(fpSpread2, j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택") + "|0#"
									+ SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량") + "|3");
							}

						}

						for (int j = 0; j <= fpSpread2.Sheets[0].Rows.Count - 1; j++)
						{
							fpSpread2.ActiveSheet.Rows[j].Visible = false;
						}
					}
				}
				else 
		...
		private string SaveExec_Detail()
		{
			...
			string strPrePrjNo = string.Empty;
					...
					for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
					{
						...
						if (strHead.Length > 0)
						{
							...
							if (ERRCode == "OK")
							{
								...
								// lot 처리시 중복 처리 방지를 위하여)
								if (string.Compare(strPrePrjNo, fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text, true) != 0)
								{
									#region Lot 정보 저장
									...
									if (string.Compare(strLotYn, "True", true) == 0 && string.IsNullOrEmpty(strTranNo) == false)
									{
										...
										for (int j = 0; j <= fpSpread2.Sheets[0].Rows.Count - 1; j++)
										{
											...
											if (string.Compare(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text,
												fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "프로젝트")].Text, true) == 0)
											{

												if (string.Compare(fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택2")].Text,
												fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택")].Text, true) != 0)
												{

													if (string.Compare(fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택")].Text, "True", true) == 0)
														strGbn = "I1";
													else
														strGbn = "D1";
													...
													strSql += ",@pBAR_CODE    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "바코드")].Text + "' ";
													strSql += ",@pMVMT_NO     = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "입고번호")].Text + "' ";
													strSql += ",@pMVMT_SEQ    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "입고순번")].Text + "' ";

													// 출고 삭제의 경우 이동후 수불번호(MOV_TRAN_NO)가 기준이 되므로 이에 대한 처리를 달리해줘야 함
													if (string.Compare(strGbn, "D1", true) == 0)
													{
														strSql += ",@pOUT_TRAN_NO     = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "수불번호")].Text + "' ";
														strSql += ",@pOUT_TRAN_SEQ    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "수불순번")].Text + "' ";
													}
													else
													{
													...
													strSql += ",@pLOT_NO          = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "Lot No")].Text + "' ";
													strSql += ",@pOUT_PROJECT_NO  = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "프로젝트")].Text + "' ";
													strSql += ",@pOUT_PROJECT_SEQ = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "차수")].Text + "' ";
													strSql += ",@pOUT_QTY         = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "참조수량")].Value + "' ";
													strSql += ",@pSTOCK_UNIT      = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "요청단위")].Text + "' ";
													strSql += ",@pREMARK          = '" + strMovTranNo + "/" + strMovTranSeq + "' ";
													strSql += ",@pIN_ID           = '" + SystemBase.Base.gstrUserID + "' ";
													strSql += ",@pUP_ID           = '" + SystemBase.Base.gstrUserID + "' ";
													strSql += ",@pORG_MVMT_NO	  = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "원본 입고번호")].Text + "' ";
													strSql += ",@pORG_MVMT_SEQ	  = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "원본 입고순번")].Text + "' ";
													...
													{
														#region lot history
														string strSqlLog = "usp_T_LOT_INOUT_HISTORY_CUDR ";
														strSqlLog += " @pTYPE		= 'I1' ";
														strSqlLog += ",@CONTENTS	= '" + strSql.Replace("'", " ") + "' ";	// LOT 입출고 내용
														strSqlLog += ",@IN_ID		= '" + SystemBase.Base.gstrUserID + "' ";	// 등록자
														strSqlLog += ",@FROM_DLL	= 'MRQ003P3' ";								// 소스 DLL
														strSqlLog += ",@IN_OUT_TYPE	= 'O' ";									// 입고('I'), 출고('O')
														strSqlLog += ",@CO_CD		= '" + SystemBase.Base.gstrCOMCD + "' ";	// 
														strSqlLog += ",@ITEM_CD		= '" + txtItemCd.Text + "' ";				// 

														DataSet dsLog = SystemBase.DbOpen.TranDataSet(strSqlLog, dbConn, Trans);
														#endregion
													}
													...
													if (string.Compare(strGbn, "D1", true) == 0)
													{
														strSql += ",@pBAR_CODE    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "이동후 바코드")].Text + "' ";
														strSql += ",@pMVMT_NO     = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "이동후 입고번호")].Text + "' ";
														strSql += ",@pMVMT_SEQ    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "이동후 입고순번")].Text + "' ";
													}
													else
													{
														strSql += ",@pBAR_CODE    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "바코드")].Text + "' ";
													...
													strSql += ",@pRCPT_QTY    = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "참조수량")].Value + "' ";
													strSql += ",@pIN_TRAN_NO  = '" + strMovTranNo + "' ";
													strSql += ",@pIN_TRAN_SEQ = '" + strMovTranSeq + "' ";
													strSql += ",@pIN_TRAN_QTY = " + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "참조수량")].Value + " ";
													strSql += ",@pSTOCK_QTY   = 0 ";
													strSql += ",@pSTOCK_UNIT  = '" + fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "요청단위")].Text + "' ";
													strSql += ",@pEND_YN      = 'N' ";
													...
													{
														#region lot history
														string strSqlLog2 = "usp_T_LOT_INOUT_HISTORY_CUDR ";
														strSqlLog2 += " @pTYPE			= 'I1' ";
														strSqlLog2 += ",@CONTENTS		= '" + strSql.Replace("'", " ") + "' ";		// LOT 입출고 내용
														strSqlLog2 += ",@IN_ID			= '" + SystemBase.Base.gstrUserID + "' ";	// 등록자
														strSqlLog2 += ",@FROM_DLL		= 'MRQ003P3' ";								// 소스 DLL
														strSqlLog2 += ",@IN_OUT_TYPE	= 'I' ";									// 입고('I'), 출고('O')
														strSqlLog2 += ",@CO_CD			= '" + SystemBase.Base.gstrCOMCD + "' ";	// 
														strSqlLog2 += ",@ITEM_CD		= '" + txtItemCd.Text + "' ";				// 

														DataSet dsLog2 = SystemBase.DbOpen.TranDataSet(strSqlLog2, dbConn, Trans);
														#endregion
													}
							...
							strPrePrjNo = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text;
		...
		private string SaveExec_Master()
		{
			...
			if (UIForm.FPMake.FPUpCheck(fpSpread1, false) == true)
			{
				...
				try
				{
					...
					strSql += ", @pUP_ID = '" + SystemBase.Base.gstrUserID + "'";
                    strSql += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD.ToString() + "' ";        // 2017.07.03. hma 추가: 회사코드
		...
		private void Sum_RefQty()
		{
            ...
            if (dReqQty < sum)
            {
                ...
                if (dRefQty != 0)
                    ...
                    //txtRefQty.Value = 0;
                    txtRefQty.Value = sum - Convert.ToDecimal(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value);
                // 2015.09.09 요구수량을 넘으면서 참조수량SUM이 초기화되었었는데 다시 계산함. (입력한 SUM에서 마지막에 등록한 참조수량을 뺌) 

                isCheck = false;

                if (fpSpread1.Sheets[0].RowHeader.Cells[fpSpread1.Sheets[0].ActiveRowIndex, 0].Text == "U")
                {
                    fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, Col].Value = 0;
                    fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value = 0;
                    isCheck = true;
                }
            }
            else
		...
		{
			int Col1 = SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량");
			int Col2 = SystemBase.Base.GridHeadIndex(GHIdx1, "선택");

            if (Column == Col1) // 참조수량이 바뀌었을때
            {
                if (Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, Col1].Value) >
                    Convert.ToDecimal(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "재고수량")].Value))
                {
                    ...
                    fpSpread1.Sheets[0].Cells[Row, Col1].Value = 0;
                    fpSpread1.ActiveSheet.SetActiveCell(Row, Col1);
                }
                else
                {
			...
			if (Column == Col2)  // 선택 했을때 
			{
                sum = 0;

                for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                {
                    if (fpSpread1.Sheets[0].Cells[i, Col2].Text == "True")
                        sum += Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value);
                }

                if (fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, Col2].Text == "True")
                {

                    // 선택에 체크하면 요구수량만큼 재고수량을 참조수량에 자동입력 2015.09.09
                    if (dReqQty > sum && fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, Col2].Text == "True")
                    {
                        if (Convert.ToDecimal(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "재고수량")].Value) > (dReqQty - sum))
                        {
                            fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value = (dReqQty - sum);
                            txtRefQty.Value = sum + (dReqQty - sum);
                        }
                        else
                        {
                            fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value =
                                                    fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "재고수량")].Value;
                            txtRefQty.Value = sum + Convert.ToDecimal(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "재고수량")].Value);
                        }
                    }
                    else if (dReqQty <= sum || fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, Col2].Text == "false")
                    {
                        DialogResult dsMsg = MessageBox.Show("참조수량이 요구수량을 초과합니다!", SystemBase.Base.MessageRtn("Z0003"), MessageBoxButtons.OK, MessageBoxIcon.Warning);

                        fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "선택")].Value = false;
                        fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value = 0;
                        txtRefQty.Value = sum;
                        isCheck = false;
                    }
                ...
                else
                {
                    fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value = 0;
                    txtRefQty.Value = sum;
                    //isCheck = false;      // 2015.10.27. hma 수정: 조회 행 전체가 체크 안된 경우에도 저장되도록 처리하기 위해 주석 처리함.
                }
				AutoCalc();
			}
		...
		private void fpSpread1_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
		{
			...
			try
			{
                ...
                // strProjectNo-자신의 프로젝트와 같으면 보이지 않게 한다.   2015.09.09 COMMENT
                // Lot는 차수관리를 하지 않기때문에 같은 프로젝트간 이동은 무의미 하다. 그래서 그리드1에 재고이동만하고, 그리드2에 Lot는 이동하지 않기위해 안보이게 함. 
				if (string.Compare(strProjectNo, fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text, true) != 0)
				{
					...
					if (fpSpread2.Sheets[0].Rows.Count > 0)
					{
						for (int j = 0; j <= fpSpread2.Sheets[0].Rows.Count - 1; j++)
						{
							...
							if (string.Compare(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text,
								fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "프로젝트")].Text, true) == 0)
							{
								fpSpread2.ActiveSheet.Rows[j].Visible = true;
							}
						}
					}
		...
		#region 참조수량 => lot 참조수량 반영
		private void AutoCalc()
		{

			decimal dTQty = 0;	// 총 참조수량
			decimal dRQty = 0;	// 잔량(총 참조수량 - 현재고량)
			decimal dSQty = 0;	// 현재고
			decimal dQQty = 0;
			...
			try 
			{
				...
				// 같은 프로젝트이고 차수가 다르면 수불그리드에서는 서로 다른 처리건이 되지만, lot 그리드에서는 동일한 프로젝트로 처리하므로 총 참조 요구수량을 합산해서 lot 참조 수량을 처리함
				for (int i = 0; i <= fpSpread1.Sheets[0].Rows.Count - 1; i++)
				{
					...
					if (string.Compare(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "선택")].Text, "true", true) == 0)
					{
						...
						if (string.Compare(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text,
							fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx2, "프로젝트")].Text, true) == 0)
						{

							dQQty = dQQty + Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value);
						}
				...
				// lot 참조수량 반영				
				if (string.Compare(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "선택")].Text, "true", true) == 0)
				{
					//dTQty = Convert.ToDecimal(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "참조수량")].Value);
					...
					if (fpSpread2.Sheets[0].Rows.Count > 0)
					{
						...
						for (int j = 0; j <= fpSpread2.Sheets[0].Rows.Count - 1; j++)
						{
							...
							dSQty = Convert.ToDecimal(fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "재고수량")].Value);

							if (string.Compare(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text,
								fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "프로젝트")].Text, true) == 0 && dRQty > 0)
							{

								fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택")].Value = true;

								if (dSQty < dRQty)
								{
									fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "참조수량")].Value = dSQty;
									dRQty = dRQty - dSQty;
								}
								else
								{
									fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "참조수량")].Value = dRQty;
									break;
								}
							}
						}
				...
				else
				{
					for (int j = 0; j <= fpSpread2.Sheets[0].Rows.Count - 1; j++)
					{
						...
						if (string.Compare(fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트")].Text,
							fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "프로젝트")].Text, true) == 0)
						{
							fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "선택")].Value = false;
							fpSpread2.Sheets[0].Cells[j, SystemBase.Base.GridHeadIndex(GHIdx2, "참조수량")].Value = 0;
						}
					}
				}
			}

...
namespace AE.ACE020
{
    public partial class ACE020 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region Form Load시
        private void ACE020_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        private void detail_SearchExec(int item_su)
        {
            try
            {
                ...
                string strWorkItemGbn = "";

                if (rdoTuip.Checked == true) { strWorkItemGbn = "1"; }
                else if (rdoIpgo.Checked == true) { strWorkItemGbn = "3"; }
                
                string strQuery = " usp_ACE020 'S1'  ";
                ...
                fpSpread1.Sheets[0].SetColumnMerge(SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드"), FarPoint.Win.Spread.Model.MergePolicy.Always);
                fpSpread1.Sheets[0].SetColumnMerge(SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명"), FarPoint.Win.Spread.Model.MergePolicy.Always);
                ...
                //    fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드")].CellType = new TextCellType();
                //    fpSpread1.Sheets[0].Columns.Get(SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드")).CellType = textCellType1;
                //    fpSpread1.Sheets[0].SetColumnMerge(SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드"), FarPoint.Win.Spread.Model.MergePolicy.Always);
                //    fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드")].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Top;

                //    fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")].CellType = new TextCellType();
                //    fpSpread1.Sheets[0].Columns.Get(SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")).CellType = textCellType1;
                //    fpSpread1.Sheets[0].SetColumnMerge(SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명"), FarPoint.Win.Spread.Model.MergePolicy.Always);
                //    fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Top;
                ...
                //    fpSpread1.Sheets[0].Columns.Get(SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트명")).CellType = textCellType1;
                ...
                //    fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "계약금액(부가세별도)")].CellType = new TextCellType();
                //    fpSpread1.Sheets[0].Columns.Get(SystemBase.Base.GridHeadIndex(GHIdx1, "계약금액(부가세별도)")).CellType = num;
                //    //fpSpread1.Sheets[0].SetColumnMerge(SystemBase.Base.GridHeadIndex(GHIdx1, "계약금액(부가세별도)"), FarPoint.Win.Spread.Model.MergePolicy.Always);
                //    //fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "계약금액(부가세별도)")].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Top;

                //    fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "매출(제품+반제품)")].CellType = new TextCellType();
                //    fpSpread1.Sheets[0].Columns.Get(SystemBase.Base.GridHeadIndex(GHIdx1, "매출(제품+반제품)")).CellType = num;
                //    //fpSpread1.Sheets[0].SetColumnMerge(SystemBase.Base.GridHeadIndex(GHIdx1, "매출(제품+반제품)"), FarPoint.Win.Spread.Model.MergePolicy.Always);
                //    //fpSpread1.Sheets[0].Columns[SystemBase.Base.GridHeadIndex(GHIdx1, "매출(제품+반제품)")].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Top;
                ...
                //    int colPlus = 0;
                //    int span_idx = 7;
                //    fpSpread1.Sheets[0].ColumnCount = 7 + item_su;
                ...
                //            if (colfirst + col_idx == SystemBase.Base.GridHeadIndex(GHIdx1, "계약금액(부가세별도)") || colfirst + col_idx == SystemBase.Base.GridHeadIndex(GHIdx1, "매출(제품+반제품)"))
                //            {
                //                fpSpread1.Sheets[0].Columns[colfirst + col_idx].Width = 120;
                //            }
                //            else
                //            {
                ...
                //            fpSpread1.Sheets[0].Cells[row_idx, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드")].Text = dt.Rows[i]["CUST_CD"].ToString();
                //            fpSpread1.Sheets[0].Cells[row_idx, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")].Text = dt.Rows[i]["CUST_NM"].ToString();
                ...
                //            fpSpread1.Sheets[0].Cells[row_idx, SystemBase.Base.GridHeadIndex(GHIdx1, "계약금액(부가세별도)")].Value = dt.Rows[i]["SO_AMT_LOC"];
                //            fpSpread1.Sheets[0].Cells[row_idx, SystemBase.Base.GridHeadIndex(GHIdx1, "매출(제품+반제품)")].Value = dt.Rows[i]["BN_AMT_LOC"];
                ...
                //            else
                //            {
                //                fpSpread1.Sheets[0].Cells[rowfirst, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처코드")].RowSpan = rowspan;
                //                fpSpread1.Sheets[0].Cells[rowfirst, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")].RowSpan = rowspan;
        ...
        private void ACE020_Activated(object sender, System.EventArgs e)
        {
            if (form_act_chk == false) dtpDtFr.Focus();
        }

        private void ACE020_Deactivate(object sender, System.EventArgs e)
        {
            form_act_chk = true;
        }
        #endregion

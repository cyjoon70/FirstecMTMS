...
namespace AH.ACH018
{
    public partial class ACH018 : UIForm.FPCOMM1 
    {
        public ACH018()
        {
            InitializeComponent();
        ...
        #region Form Load 시
        private void ACH018_Load(object sender, System.EventArgs e)
        {
            SystemBase.Validation.GroupBox_Setting(groupBox1);//필수 적용
            SystemBase.ComboMake.C1Combo(cboDeprStatus, "usp_B_COMMON @pTYPE='COMM', @pCODE = 'A204', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "'", 9);   //상각상태
            ...
            dtpDeprYYYY.Value = SystemBase.Base.ServerTime("YYMMDD");
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_ACH018  'S1'";
                    strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                    strQuery += ", @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "' ";
                    strQuery += ", @pDEPR_YYYY = '" + dtpDeprYYYY.Text + "' ";
                    if(optSlipY.Checked == true) strQuery += ", @pSLIP_YN = 'Y' ";
                    else if (optSlipN.Checked == true) strQuery += ", @pSLIP_YN = 'N' ";
                    strQuery += ", @pDEPR_STATUS = '" + cboDeprStatus.SelectedValue.ToString() + "' ";
                    ...
                    {
                        strQuery += ", @pDISPOSAL_YN = 'Y' ";
                    }
                    if (chkCu_Disposal.Checked == true)
                    {
                        strQuery += ", @pCU_DISPOSAL_YN = 'Y' ";
                    }

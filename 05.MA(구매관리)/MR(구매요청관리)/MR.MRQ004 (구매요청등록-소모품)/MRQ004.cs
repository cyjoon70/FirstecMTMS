...
namespace MR.MRQ004
{
    public partial class MRQ004 : UIForm.FPCOMM2
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void MRQ004_Load(object sender, System.EventArgs e)
        {
            //그룹박스 필수체크
            ...
            //기타 세팅
            dtpSReqDtFr.Value = Convert.ToDateTime(SystemBase.Base.ServerTime("YYMMDD")).AddMonths(-1).ToShortDateString();
            dtpSReqDtTo.Value = Convert.ToDateTime(SystemBase.Base.ServerTime("YYMMDD")).ToShortDateString();
            dtpReqDt.Value = Convert.ToDateTime(SystemBase.Base.ServerTime("YYMMDD")).ToShortDateString();
        ...
        private void Search(string strReqNo)
        {
            ...
            try
            {
                if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
                {
                    ...
                    string strQuery = " usp_MRQ004  @pTYPE = 'S1'";
        ...
        private void SubSearch(string strCode)
        {
            ...
            try
            {
                ...
                //				txtPurOrgCd.Text = dt.Rows[0]["PUR_ORG"].ToString();
                //				txtPurOrgNm.Text = dt.Rows[0]["PUR_ORG_NM"].ToString();
                txtRemark.Value = dt.Rows[0]["REMARK"].ToString();

                //Detail그리드 정보.
                string strSql1 = " usp_MRQ004  'S3' ";
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "전용여부")].Text == "N") //프로젝트, 차수 realonly
                            ...
                            UIForm.FPMake.grdReMake(fpSpread1, i,
                                ...
                                + "#" + SystemBase.Base.GridHeadIndex(GHIdx1, "제품코드") + "|0"
                    ...
                    if (fpSpread1.Sheets[0].Rows.Count > 0)
                    {
                        ...
                        try
                        {
                            if (NewFlg != 0)
                            {
                                ...
                                string strSql = " usp_MRQ004 '" + strMstType + "'";
                            ...
                            if ((SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false) == true))// 그리드 필수항목 체크 
                            {
                                ...
                                if (DelCheck() == false)
                                {
                                    ...
                                    if (dsMsg == DialogResult.Yes)
                                    {
                                        try
                                        {
                                            string strDelSql = " usp_MRQ004  'D1'";
                                ...
                                for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                                {
                                    ...
                                    if (strHead.Length > 0)
                                    {
                                        ...
                                        string Query = " usp_M_COMMON @pTYPE = 'M012', @pCODE = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "품목코드")].Text + "', @pNAME = '" + cboPlantCd.SelectedValue.ToString() + "', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                                        DataTable dt = SystemBase.DbOpen.NoTranDataTable(Query);

                                        if (dt.Rows.Count > 0)
                                        {
                                            if (dt.Rows[0]["ITEM_ACCT"].ToString() != "Z1")
                                            {
                                                if (Convert.ToInt16(dt.Rows[0]["ITEM_ACCT"].ToString()) < 60)
                                                {
                                                    ERRCode = "ER";
                                                    MSGCode = "품목계정이 10(제품) ~ 50(상품)인 품목은 저장할 수 없습니다! 구매요청등록에서 하세요!";
                                        ...
                                        //											+ "' , @pCODE = '"  + fpSpread1.Sheets[0].Cells[i,SystemBase.Base.GridHeadIndex(GHIdx1,"제품코드")].Text+ "'";
                                        //
                                        //										
                                        ...
                                        string strSql = " usp_MRQ004 '" + strGbn + "'";
                                        strSql += ", @pREQ_NO = '" + strAutoReqNo + "' ";
                                        ...
                                        //										if(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "도면REV.")].Text.Trim() != "")
        ...
        protected override void fpButtonClick(int Row, int Column)
        {
            ...
            if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "품목코드_2"))
            {
                try
                {
                    WNDW.WNDW005 pu = new WNDW.WNDW005(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "품목코드")].Text, "70");
                    pu.ShowDialog();
                    if (pu.DialogResult == DialogResult.OK)
                    {
                        string[] Msgs = pu.ReturnVal;
                        if (Msgs[5].ToString() != "Z1")
                        {
                            if (Convert.ToInt16(Msgs[5].ToString()) < 60)
                            {
                                MessageBox.Show("품목계정이 10(제품) ~ 50(상품)인 품목은 저장할 수 없습니다! 구매요청등록에서 하세요!", SystemBase.Base.MessageRtn("Z0003"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            //							fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "입고가능일")].Text = Convert.ToDateTime(dtpReqDt.Value).AddDays(Convert.ToDouble(iLT2)).ToShortDateString();
            ...
            else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트번호_2"))
            {
                try
                {
                    WNDW.WNDW007 pu = new WNDW.WNDW007(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트번호")].Text, "N");
                    pu.ShowDialog();
                    if (pu.DialogResult == DialogResult.OK)
                    {
                        string[] Msgs = pu.ReturnVal;
                    ...
                    //					string strQuery = " usp_M_COMMON 'P002' ";										// 쿼리
                    //					string[] strWhere=new string[]{"@pCODE","@pNAME"};				// 쿼리 인자값(조회조건)
                    //					string[] strSearch=new string[]{fpSpread1.Sheets[0].Cells[Row,SystemBase.Base.GridHeadIndex(GHIdx1,"프로젝트번호")].Text, ""};		// 쿼리 인자값에 들어갈 데이타
                    //
                    //					//UIForm.PopUpSP pu = new UIForm.PopUpSP(strQuery, strWhere, strSearch, PHeadText7, PTxtAlign7, PCellType7, PHeadWidth7, PSearchLabel7);
                    //					UIForm.FPPOPUP pu = new UIForm.FPPOPUP("P00002", strQuery, strWhere, strSearch, new int[]{0,1}, "프로젝트 조회", false);
                    //					pu.Width = 500;
                    ...
                    //						fpSpread1.Sheets[0].Cells[Row,SystemBase.Base.GridHeadIndex(GHIdx1,"프로젝트차수")].Text	= Msgs[2].ToString();
                    //
                    //						UIForm.FPMake.fpChange(fpSpread1, Row);//수정플래그
                    //					}
            ...
            else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "제품코드_2"))
            {
                try
                {
                    ...
                    //UIForm.PopUpSP pu = new UIForm.PopUpSP(strQuery, strWhere, strSearch, PHeadText7, PTxtAlign7, PCellType7, PHeadWidth7, PSearchLabel7);
                    UIForm.FPPOPUP pu = new UIForm.FPPOPUP("P00003", strQuery, strWhere, strSearch, new int[] { 0, 1 }, "제품 조회", false);
                    pu.Width = 400;
                    pu.ShowDialog();	//공통 팝업 호출
        ...
        private void txtSDeptCd_TextChanged(object sender, EventArgs e)
        {
            if (strBtn == "N")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    txtSReorgId.Value = dt.Rows[0][0].ToString();
                }
                else
                {
                    txtSReorgId.Value = "";
        ...
        #region 버튼 Click
        private void butSUser_Click(object sender, EventArgs e)
        {
            strBtn = "Y";
            try
            {
        ...
        private void butSDept_Click(object sender, EventArgs e)
        {
            strBtn = "Y";
            try
            {
        ...
        private void btnProj_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                WNDW.WNDW007 pu = new WNDW.WNDW007(txtProjNo.Text);
                pu.ShowDialog();
                if (pu.DialogResult == DialogResult.OK)
                {
                    string[] Msgs = pu.ReturnVal;
        ...
        #region MRQ004_Activated
        private void MRQ004_Activated(object sender, EventArgs e)
        {
            if (form_act_chk == false) dtpSReqDtFr.Focus();
        }

        private void MRQ004_Deactivate(object sender, EventArgs e)
        {
            form_act_chk = true;
        }
        #endregion

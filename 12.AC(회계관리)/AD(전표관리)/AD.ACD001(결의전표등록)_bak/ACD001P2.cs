...
namespace AD.ACD001
{
    public partial class ACD001P2 : UIForm.FPCOMM2
    {
        #region 변수선언
        DataTable Dt = null;
        string strCUR_CD = "";
        ...
        public ACD001P2(DataTable Ar_Dt, string CUR_CD, string CUST_CD, string BIZ_AREA_CD, string ACCT_CD, string ACCT_NM)
        {
            Dt = Ar_Dt;
            strCUR_CD = CUR_CD;
            strCustCd = CUST_CD;
        ...
        #region Form Load 시
        private void ACD001P2_Load(object sender, System.EventArgs e)
        {
            try
            {
        ...
        protected override void NewExec()
        {
            try
            {
                ...
                dtpArDtFrom.Text = Convert.ToDateTime(YYMMDD).AddMonths(-3).ToShortDateString();
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                ...
                {
                    string strQuery2 = " usp_ACD001  'P71'";
                    string strQuery = " usp_ACD001  'P7'";
                    strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                    strQuery += ", @pCUST_CD = '" + txtCustCd.Text + "' ";
                    strQuery += ", @pAR_DT_FROM = '" + dtpArDtFrom.Text + "' ";
                ...
                if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false) == true)
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count - 1; i++)
                    {
                        if(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "반제액")].Text != "")
                            iSlipAmt += Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "반제액")].Value);
                        if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "할인액")].Text != "")
                            iSlipAmt += Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "할인액")].Value);

                        if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "반제액(자)")].Text != "")
                            iSlipAmtLoc += Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "반제액(자)")].Value);
                        if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "할인액(자)")].Text != "")
                            iSlipAmtLoc += Convert.ToDecimal(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "할인액(자)")].Value);
                    ...
                    if (fpSpread2.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx2, " ")].Text == "True")
                    {
                        if (fpSpread1.Sheets[0].Rows.Count > 0)
                        {
                            if (fpSpread1.Sheets[0].RowHeader.Cells[fpSpread1.Sheets[0].Rows.Count - 1, 0].Text == "합계")
                                fpSpread1.Sheets[0].SetActiveCell(fpSpread1.Sheets[0].Rows.Count - 2, SystemBase.Base.GridHeadIndex(GHIdx1, "채권번호"));
                        }
                ...
                if (Dt != null)
                {
                    if (Dt.Rows.Count > 0)
                    {
                        for (int iDtRow = 0; iDtRow < Dt.Rows.Count; iDtRow++)
                        {
                            ...
                            for (int iRow = 0; iRow < fpSpread2.Sheets[0].Rows.Count; iRow++)
                            {
                                if (Dt.Rows[iDtRow]["AR_NO"].ToString() == fpSpread2.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx2, "채권번호")].Text)
                                {
                                    fpSpread2.Sheets[0].Rows.Remove(iRow, 1);
                                    continue;
                                }
        ...
        public DataTable Ar_Dt { get { return Dt; } set { Dt = value; } }
        ...
        public Decimal SLIP_AMT { get { return iSlipAmt; } set { iSlipAmt = value; } }
        public Decimal SLIP_AMT_LOC { get { return iSlipAmtLoc; } set { iSlipAmtLoc = value; } }

...
namespace AC.ACC001
{
    public partial class ACC001 : UIForm.FPCOMM2
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void ACC001_Load(object sender, System.EventArgs e)
        {
            NewExec();
        }
        #endregion
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_ACC001  'S1'";
                    strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                    strQuery += ", @pAP_DT_FROM = '" + dtpArDtFr.Text + "' ";
                    strQuery += ", @pAP_DT_TO = '" + dtpArDtTo.Text + "' ";
                    strQuery += ", @pCUST_CD = '" + txtCustCd.Text + "' ";

                    if (dtpArExpDtFr.Text == "")
                    {
                        strQuery += ", @pAP_EXP_DT_FROM = '1900-01-01' ";
                    }
                    else
                    {
                        strQuery += ", @pAP_EXP_DT_FROM = '" + dtpArExpDtFr.Text + "' ";
                    }
                    if (dtpArExpDtTo.Text == "")
                    {
                        strQuery += ", @pAP_EXP_DT_TO = '2999-12-31' ";
                    }
                    else
                    {
                        strQuery += ", @pAP_EXP_DT_TO = '" + dtpArExpDtTo.Text + "' ";
                    }
        ...
        private void fpSpread2_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            try
            {
                ...
                string strAP_NO = fpSpread2.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx2, "채무번호")].Text;
                CLS_Search(strAP_NO);
        ...
        #region 반제정보 조회
        private void CLS_Search(string AP_NO)
        {
            this.Cursor = Cursors.WaitCursor;
            try
            {
                //상세조회 SQL
                string strQuery = " usp_ACC001  'S2'";
                strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                strQuery = strQuery + ", @pAP_NO ='" + AP_NO + "' ";

...
namespace PC.PCC031
{
    public partial class PCC031 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region Form Load시
        private void PCC031_Load(object sender, System.EventArgs e)
        {
            //필수체크
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            if (strStartWoNo != strNewWoNo)
                            {
                                ...
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "차수")].Text = ds.Tables[0].Rows[i][1].ToString();  //차수
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "제품코드")].Text = ds.Tables[0].Rows[i][2].ToString();  //제품코드
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "제품명")].Text = ds.Tables[0].Rows[i][3].ToString();  //제품명
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "제품완료일")].Text = ds.Tables[0].Rows[i][4].ToString();  //납기일
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "수주납기일")].Text = ds.Tables[0].Rows[i][5].ToString();  //수주납기일
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "수주납기일(참조)")].Text = ds.Tables[0].Rows[i][6].ToString();  //수주납기일(참조)
                                ...
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "시작일")].Text = ds.Tables[0].Rows[i][14].ToString();  //작업시작일
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "완료일")].Text = ds.Tables[0].Rows[i][15].ToString();  //작업종료일
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "오더수량")].Text = ds.Tables[0].Rows[i][16].ToString(); //오더수량
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "잔량")].Text = ds.Tables[0].Rows[i][17].ToString(); //잔여수량
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "비고")].Text = ds.Tables[0].Rows[i][23].ToString(); //비고
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "작업장")].Text = ds.Tables[0].Rows[i][25].ToString(); //작업장
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "완료율(%)")].Text = "";
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "1")].Text = ds.Tables[0].Rows[i][21].ToString(); //작업명


                                if (ds.Tables[0].Rows[i][19].ToString() == "ST") //공정실적상태
                                    fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "1")].ForeColor = Color.Blue;
                                else if (ds.Tables[0].Rows[i][19].ToString() == "CL")
                                {
                            ...
                            {
                                fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "1") + intItemCount].Text = ds.Tables[0].Rows[i][21].ToString();

                                if (ds.Tables[0].Rows[i][19].ToString() == "ST")
                        ...
                        fpSpread1.Sheets[0].SetActiveCell(0, SystemBase.Base.GridHeadIndex(GHIdx1, "프로젝트"));
        ...
        private void btnItemSpec_Click(object sender, System.EventArgs e)
        {
            if (fpSpread1.Sheets[0].Rows.Count > 0)
            {
                ...
                PCC007P2 form = new PCC007P2(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text, "");
                form.ShowDialog();
            }
        }
        #endregion

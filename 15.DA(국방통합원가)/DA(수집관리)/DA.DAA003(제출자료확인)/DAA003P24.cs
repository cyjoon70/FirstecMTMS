#region DAA003P24 작성 정보
/*************************************************************/
// 단위업무명 : 상각대상경비 엑셀 파일 업로드 처리
// 작 성 자 :   유재규
// 작 성 일 :   2012-11-07
// 작성내용 :   
// 수 정 일 :
...
namespace DA.DAA003
{
    public partial class DAA003P24 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region DAA003P24()
        public DAA003P24(string ORDR_YEAR, string DCSN_NUMB, string CALC_DEGR, string DPRT_CODE, 
        ...
        #region DAA003P24_Load
        private void DAA003P24_Load(object sender, EventArgs e)
        {
        ...
        private void FpGrid_DataSet()
        {
            try
            {
                G1Etc[SystemBase.Base.GridHeadIndex(GHIdx1, "비목구분")] = SystemBase.ComboMake.ComboOnGrid("usp_B_COMMON @pType='COMM', @pCODE = 'D031', @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "'", 0);
                ...
                FrozenCol = 3; // 고정컬럼(세부항목명 까지)

                strSql = " usp_DAA003_상각대상경비  ";
                strSql += "  @pTYPE = 'S1'";
        ...
        private void txtM_UNIT_TextChanged(object sender, EventArgs e)
        {
            try
            {
                string strSql = "AND COMP_CODE =  'SYS' AND MAJOR_CODE IN ( 'CO020','CO021','CO022' ) ";
        ...
        #region DAA003P24_Shown
        private void DAA003P24_Shown(object sender, EventArgs e)
        {
            SearchExec();
        }
        #endregion

...
namespace CW.CWB005
{
    public partial class CWB005 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region Form Load시
        private void CWB005_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        protected override void SearchExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                ...
                try
                {
                    ...
                    string strQuery = "usp_CWB005 @pTYPE = 'S1'";
        ...
        private void Set_Section()
        {
            ...
            for (int i = 6; i < 18; i++)
            {
                string strDtFr = Convert.ToDateTime(dtpWorkDtFr.Value).AddMonths(i - 6).ToString().Substring(0,7);
            ...
            for (int i = 0; i < iCnt; i++)
            {
                ...
                if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Text == "")
                {
                    ...
                    fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부품명")].ColumnSpan = 3;
                    fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부품명")].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;

                    //소계 합계 색 변경
                    for (int j = 1; j < fpSpread1.Sheets[0].ColumnCount; j++)
                    {
                        if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부품명")].Text == "합계")
                    ...
                    fpSpread1.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx1, "부품명")].RowSpan = i - iRow;
                    fpSpread1.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx1, "부품코드")].RowSpan = i - iRow;
        ...
        protected override void ExcelExec()
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    oWorkSheet.Cells[2, iColumn - 2] = "인쇄일자";
                    ...
                    {
                        // header 저장
                        oWorkSheet.Cells[4, j] = fpSpread1.Sheets[0].ColumnHeader.Cells[0, j].Text;
                    }
                    ...
                    for (int rowNo = 0; rowNo < fpSpread1.Sheets[0].Rows.Count; rowNo++)
                    {
                        ...
                        for (int colNo = 3; colNo < fpSpread1.Sheets[0].Columns.Count; colNo++)
                        {
                            ...
                            if (fpSpread1.Sheets[0].Cells[rowNo, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Text == "")
                            {
                                ...
                                //소계 합계 색 변경
                                if (fpSpread1.Sheets[0].Cells[rowNo, SystemBase.Base.GridHeadIndex(GHIdx1, "부품명")].Text == "합계")
                    ...
                    //헤드 색지정,테두리 설정
                    oRange = oWorkSheet.get_Range("B4", "R4");

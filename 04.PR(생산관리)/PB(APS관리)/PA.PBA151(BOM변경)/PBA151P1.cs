...
namespace PA.PBA151
{
    public partial class PBA151P1 : UIForm.FPCOMM1
    {
        ...
        #region 생성자
        public PBA151P1(string Item_Cd, string Item_Nm, string Rev_No, string PRNT_BOM_NO, string BOM_DEV_USR_ID, string BOM_MFG_USR_ID, string BOM_QUR_USR_ID, string BOM_APP_USR_ID, string Child)
        {
            strItem_Cd = Item_Cd;
            strItem_Nm = Item_Nm;
            strRev_No = Rev_No;
        ...
        protected override void SearchExec()
        {
            ...
            for (int i = 0; i < Msgs.Length; i++)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    fpSpread1.Sheets[0].Cells[i, 2].Text = dt.Rows[0]["CHILD_ITEM_CD"].ToString();		//품목코드
                    fpSpread1.Sheets[0].Cells[i, 3].Text = dt.Rows[0]["ITEM_NM"].ToString();			//품목명
                    fpSpread1.Sheets[0].Cells[i, 4].Text = dt.Rows[0]["ITEM_SPEC"].ToString();			//규격

                    fpSpread1.Sheets[0].Cells[i, 5].Text = dt.Rows[0]["CHILD_ITEM_QTY"].ToString();		//자품목기준수
                    fpSpread1.Sheets[0].Cells[i, 6].Value = dt.Rows[0]["CHILD_ITEM_UNIT"].ToString();	//단위
                    fpSpread1.Sheets[0].Cells[i, 7].Text = dt.Rows[0]["MAT_SIZE"].ToString();			//재료규격
                    fpSpread1.Sheets[0].Cells[i, 8].Text = dt.Rows[0]["VALID_FROM_DT"].ToString();		//유효일자(FROM)
                    fpSpread1.Sheets[0].Cells[i, 9].Text = dt.Rows[0]["VALID_TO_DT"].ToString();		//유효일자(TO)			

                    fpSpread1.Sheets[0].Cells[i, 10].Text = dt.Rows[0]["CHILD_ITEM_CD"].ToString();		//품목코드
                    fpSpread1.Sheets[0].Cells[i, 12].Text = dt.Rows[0]["ITEM_NM"].ToString();			//품목명
                    fpSpread1.Sheets[0].Cells[i, 13].Text = dt.Rows[0]["ITEM_SPEC"].ToString();			//규격

                    fpSpread1.Sheets[0].Cells[i, 14].Text = dt.Rows[0]["CHILD_ITEM_QTY"].ToString();	//자품목기준수
                    fpSpread1.Sheets[0].Cells[i, 15].Value = dt.Rows[0]["CHILD_ITEM_UNIT"].ToString();	//단위
                    fpSpread1.Sheets[0].Cells[i, 16].Text = dt.Rows[0]["MAT_SIZE"].ToString();			//재료규격
                    fpSpread1.Sheets[0].Cells[i, 17].Text = dt.Rows[0]["VALID_FROM_DT"].ToString();		//유효일자(FROM)
                    fpSpread1.Sheets[0].Cells[i, 18].Text = dt.Rows[0]["VALID_TO_DT"].ToString();		//유효일자(TO)
                }
            }
        }
        #endregion
                ...
                try
                {
                    ...
                    if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false) == true)
                    {
                        ...
                        for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                        {
                            ...
                            strSql += ", @pCHILD_ITEM_SEQ = '" + fpSpread1.Sheets[0].Cells[i, 0].Text + "' ";	//자품목순번
                            strSql += ", @pCHILD_ITEM_CD = '" + fpSpread1.Sheets[0].Cells[i, 10].Text + "' ";	//자품목 품목코드
                            strSql += ", @pCHILD_ITEM_QTY = '" + fpSpread1.Sheets[0].Cells[i, 14].Value + "' ";	//자품목기준수
                            strSql += ", @pCHILD_ITEM_UNIT = '" + fpSpread1.Sheets[0].Cells[i, 15].Value + "' ";	//자품목단위
                            strSql += ", @pMAT_SIZE = '" + fpSpread1.Sheets[0].Cells[i, 16].Text + "' ";		//재질규격						
                            strSql += ", @pVALID_FROM_DT = '" + fpSpread1.Sheets[0].Cells[i, 17].Text + "' ";	//시작일
                            strSql += ", @pVALID_TO_DT = '" + fpSpread1.Sheets[0].Cells[i, 18].Text + "' ";		//종료일
                            strSql += ", @pREV_RESAN = '" + fpSpread1.Sheets[0].Cells[i, 19].Text + "' ";		//변경이유						
            ...
            try
            {
                if (e.Column == 11)
                {
                    ...
                    if (pu.DialogResult == DialogResult.OK)
                    {
                        ...
                        fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "자품목코드")].Text = Msgs[2].ToString();		// 자품목코드
                        fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "자품목명")].Text = Msgs[3].ToString();		// 자품목명
                        fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "규격")].Text = Msgs[7].ToString();		    // 규격
                    }
                }
            }
            catch (Exception f)

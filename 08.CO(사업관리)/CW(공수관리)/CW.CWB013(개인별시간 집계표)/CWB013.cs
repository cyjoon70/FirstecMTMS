...
namespace CW.CWB013
{
    public partial class CWB013 : UIForm.FPCOMM1
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void CWB013_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
                {
                    ...
                    for (int i = 6; i < 18; i++)
                    {
                        string strWorkDtFr = Convert.ToDateTime(dtpWorkDtFr.Value.ToString() + "-01").AddMonths(i - 6).ToString();
                    ...
                    if (fpSpread1.Sheets[0].RowCount > 0)
                    {
                        ...
                        for (int i = 0; i < fpSpread1.Sheets[0].RowCount; i++)
                        {
                            ...
                            {
                                fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부서명")].ColumnSpan = 3;
                                fpSpread1.Sheets[0].Cells[i, 0, i, fpSpread1.Sheets[0].ColumnCount - 1].BackColor = SystemBase.Base.gColor1;
                            }

                            if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "작업자명")].Text == "소계")
                            {
                                fpSpread1.Sheets[0].Cells[i, 0, i, fpSpread1.Sheets[0].ColumnCount - 1].BackColor = SystemBase.Base.gColor2;
                            }
                        }
                    }
                }

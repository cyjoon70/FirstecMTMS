...
namespace EI.EIS203
{
    public partial class EIS203 : UIForm.FPCOMM1
    {
        public EIS203()
        {
            InitializeComponent();
        ...
        #region Form Load시
        private void EIS203_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        protected override void SearchExec()
        {
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                ...
                try
                {
                    string strQuery1 = " usp_EIS203  @pTYPE = 'S1' ";
                    ...
                    else
                    {
                        ...
                        BindSeries(c1Chart1, 0, dv, "구입품", "월");
                        BindSeries(c1Chart1, 1, dv, "수입품");
                        BindSeries(c1Chart1, 2, dv, "합계");

                        fpSpread1.Sheets[0].RowCount = 3;
                        fpSpread1.Sheets[0].RowHeader.Cells[0, 0].Text = "구입품";
                        fpSpread1.Sheets[0].RowHeader.Cells[1, 0].Text = "수입품";
                        fpSpread1.Sheets[0].RowHeader.Cells[2, 0].Text = "합계";
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            fpSpread1.Sheets[0].Cells[0, i + 1].Value = dt.Rows[i]["구입품"];
                            fpSpread1.Sheets[0].Cells[1, i + 1].Value = dt.Rows[i]["수입품"];
                            fpSpread1.Sheets[0].Cells[2, i + 1].Value = dt.Rows[i]["합계"];
                        }

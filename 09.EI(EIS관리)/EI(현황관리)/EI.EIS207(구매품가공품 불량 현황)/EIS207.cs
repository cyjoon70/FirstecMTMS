...
namespace EI.EIS207
{
    public partial class EIS207 : UIForm.FPCOMM1
    {
        public EIS207()
        {
            InitializeComponent();
        ...
        #region Form Load시
        private void EIS207_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        protected override void SearchExec()
        {
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                ...
                try
                {
                    string strQuery1 = " usp_EIS207  @pTYPE = 'S1' ";
                    ...
                    else
                    {
                        ...
                        BindSeries(c1Chart1, 0, dv, "구매불량율", "월");
                        BindSeries(c1Chart1, 1, dv, "가공불량율");
                        BindSeries(c1Chart1, 2, dv, "평균불량율");
                        BindSeries(c1Chart1, 3, dv, "UCL");
                        BindSeries(c1Chart1, 4, dv, "LCL");

                        fpSpread1.Sheets[0].RowCount = 5;
                        fpSpread1.Sheets[0].RowHeader.Cells[0, 0].Text = "구매불량율";
                        fpSpread1.Sheets[0].RowHeader.Cells[1, 0].Text = "가공불량율";
                        fpSpread1.Sheets[0].RowHeader.Cells[2, 0].Text = "평균불량율";
                        fpSpread1.Sheets[0].RowHeader.Cells[3, 0].Text = "UCL";
                        fpSpread1.Sheets[0].RowHeader.Cells[4, 0].Text = "LCL";

                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            fpSpread1.Sheets[0].Cells[0, i + 1].Value = dt.Rows[i]["구매불량율"];
                            fpSpread1.Sheets[0].Cells[1, i + 1].Value = dt.Rows[i]["가공불량율"];
                            fpSpread1.Sheets[0].Cells[2, i + 1].Value = dt.Rows[i]["평균불량율"];
                            fpSpread1.Sheets[0].Cells[3, i + 1].Value = dt.Rows[i]["UCL"];
                            fpSpread1.Sheets[0].Cells[4, i + 1].Value = dt.Rows[i]["LCL"];
                        }

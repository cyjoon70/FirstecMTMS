        ...
        private void btnRev_Click(object sender, System.EventArgs e)
        {
            if (fpSpread1.Sheets[0].Rows.Count > 0)
            {
                try
                {
                    ...
                    PBA151P1 pu = new PBA151P1(strItem_Cd, strItem_Nm, strRev_No, strPRNT_BOM_NO, strBOM_DEV_USR_ID, strBOM_MFG_USR_ID, strBOM_QUR_USR_ID, strBOM_APP_USR_ID, strChild);
                    pu.ShowDialog();
                    if (pu.DialogResult == DialogResult.OK)
                    {
                        showBomInfo(NEW_NODE_TAG);

...
namespace PB.PSA027
{
    public partial class PSA027 : UIForm.FPCOMM1
    {
        ...
        private void PSA027_Load(object sender, System.EventArgs e)
        {
            ...
            rdoSoAll.Checked = true;            // 2019.09.20. hma 추가: 수주마감여부 기본값을 '전체'로
        }
        #endregion
        ...
        protected override void SearchExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                ...
                if (Convert.ToDateTime(dtpDeliveryDtFr.Value) <= Convert.ToDateTime(dtpDeliveryDtTo.Value)) // 납기일From 이 To 보다 크면 조회내용이 없다.
                {
                    try
                    {
                        // 2019.09.20. hma 추가(Start): 수주마감여부 검색조건 체크
                        string strSoClose;
                        strSoClose = "";
                        if (rdoSoAll.Checked == true)
                            strSoClose = "A";
                        else if (rdoSoIng.Checked == true)
                            strSoClose = "N";
                        else if (rdoSoClose.Checked == true)
                            strSoClose = "Y";
                        // 2019.09.20. hma 추가(End)
                        ...
                        strQuery += ", @pSO_CLOSE = '" + strSoClose + "'";     // 2019.09.20. hma 추가: 수주마감여부
                        ...
                        if (fpSpread1.Sheets[0].RowCount > 0)
                        {
                            ...
                            //그리드헤드명 바꾸기					
                            for (i = 12; i < dt.Columns.Count - 2; i++)     // 2019.09.20. hma 수정: 시작 컬럼을 11=>12로 변경
                            {
        ...
        private void Set_Section()
        {
            ...
            {
                string strCust = "";
                strCust = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처")].Text;
                if (strCust.Substring(strCust.Length -2, 2) == "소계")
                {
                ...
                if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처")].Text == "ZZZZZ합계")
                {
                    fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처")].Text = "합계";
                    fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")].Text = "합계";
                ...
                for (int j = 1; j < fpSpread1.Sheets[0].ColumnCount; j++)
                {
                    ...
                    else if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처")].Text == fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "거래처명")].Text)

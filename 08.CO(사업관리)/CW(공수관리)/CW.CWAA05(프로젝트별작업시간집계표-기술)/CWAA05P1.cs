#region 작성정보
/*********************************************************************/
// 단위업무명:  프로젝트별 작업시간 집계 데이터 엑셀 업로드 - 기술
// 작 성 자  :  한 미 애
// 작 성 일  :  2017-02-21
...
namespace CW.CWAA05
{ 
    public partial class CWAA05P1 : UIForm.FPCOMM1
    {
        #region 변수선언
        string strPLANT_CD = "";
        #endregion
        ...
        public CWAA05P1(string sPLANT_CD)
        {
            InitializeComponent();
        ...
        #region Form Load 시
        private void CWAA05P1_Load(object sender, System.EventArgs e)
        {
            this.Text = "프로젝트별공수업로드-기술";
        ...
        private bool Exists_Check()
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_CWAA05 'C1'";
                    strQuery += ", @pCO_CD ='" + SystemBase.Base.gstrCOMCD + "'";
                    strQuery += ", @pPLANT_CD ='" + strPLANT_CD + "'";
                    strQuery += ", @pDEPT_TYPE = 'E'";                      //2017.02.22 부서구분 추가
                ...
                try
                {
                    ...
                    if (ds.Tables[0].Rows.Count != 0)
                    {
                        for (int i = 1; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            strLabType = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "직간접")].Text.Trim().Replace(" ", "");
                            if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "공수")].Text.Trim().Replace(" ", "") == "")
                                fWorkHours = "0";
                            else
                                fWorkHours = fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "공수")].Text.Trim().Replace(" ", "");

                            string strSql = " usp_CWAA05 'I1 '";
                            strSql += ", @pCO_CD ='" + SystemBase.Base.gstrCOMCD + "' ";
                            strSql += ", @pPLANT_CD ='" + SystemBase.Base.gstrPLANT_CD + "' ";
                            strSql += ", @pDEPT_TYPE = 'E' ";
                            strSql += ", @pLAB_TYPE = '" + strLabType + "' ";

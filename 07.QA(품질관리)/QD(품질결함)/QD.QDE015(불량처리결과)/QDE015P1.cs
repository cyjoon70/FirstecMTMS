...
namespace QD.QDE015
{
    public partial class QDE015P1 : UIForm.FPCOMM2
    {
        #region 변수선언
        string strInspReqNo = "";
        string strItemCd = "";
        ...
        public QDE015P1(FarPoint.Win.Spread.FpSpread spread, int Row, string InspReqNo, string ItemCd)
        {
            //
        ...
        #region Form Load 시
        private void QDE015P1_Load(object sender, System.EventArgs e)
        {
            this.Text = "처리결과등록";
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                string strQuery = " usp_QDE015  @pTYPE = 'P1'";
                ...
                if (fpSpread2.Sheets[0].Rows.Count > 0)
                {
                    ...
                    { txtQdecContent.Value = fpSpread2.Sheets[0].Cells[0, SystemBase.Base.GridHeadIndex(GHIdx2, "품질판정")].Value.ToString(); }
                    else
                    { txtQdecContent.Value = ""; }

                    txtExaminerCd.Value = fpSpread2.Sheets[0].Cells[0, SystemBase.Base.GridHeadIndex(GHIdx2, "검토자")].Text;

                    if (fpSpread2.Sheets[0].Cells[0, SystemBase.Base.GridHeadIndex(GHIdx2, "처리결과내용")].Text != "")
                    { txtHresContent.Text = fpSpread2.Sheets[0].Cells[0, SystemBase.Base.GridHeadIndex(GHIdx2, "처리결과내용")].Value.ToString(); }
                    else
                    { txtHresContent.Text = ""; }

                    txtApproverCd.Text = fpSpread2.Sheets[0].Cells[0, SystemBase.Base.GridHeadIndex(GHIdx2, "승인자")].Text;
                    ...
                    if (UIForm.FPMake.FPUpCheck(fpSpread1, false) == true)
                    {
                        ...
                        for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                        {
                            ...
                            if (strHead.Length > 0)
                            {
                                ...
                                string strSql1 = " usp_QDE015 '" + strGbn + "'";
        ...
        Exit:
            ...
            if (ERRCode == "OK")
            {
                spd.Sheets[0].Cells[iRow, 16].Value = 1;
        ...
        Exit:
            ...
            if (ERRCode == "OK")
            {
                spd.Sheets[0].Cells[iRow, 16].Value = 0;
        ...
        private void SubSearch(int iRow)
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_QDE015  @pTYPE = 'P2'";
                    strQuery += ", @pINSP_REQ_NO = '" + txtInspReqNo.Text + "'";
                    if (strProcSeq == "")
                    { strQuery += ", @pPROC_SEQ = '*'"; }
                    else

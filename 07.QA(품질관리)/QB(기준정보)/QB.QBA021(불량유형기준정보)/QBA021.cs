...
namespace QB.QBA021
{
    public partial class QBA021 : UIForm.FPCOMM1
    {
        #region 생성자
        public QBA021()
        {
        ...
        #region Form Load 시
        private void QBA021_Load(object sender, System.EventArgs e)
        {
            //필수 항목 체크
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                string strQuery = " usp_QBA021 @pTYPE = 'S1'";
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead.Length > 0)
                        {
                            ...
                            string strQuery = " usp_QBA021 @pTYPE = '" + strGbn + "'";
                            strQuery += ", @pINSP_CLASS_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "검사분류코드")].Value + "'";
                            strQuery += ", @pDEFECT_TYPE_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "불량유형코드")].Text + "'";
                            strQuery += ", @pDEFECT_TYPE_NM = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "불량유형명")].Text + "'";

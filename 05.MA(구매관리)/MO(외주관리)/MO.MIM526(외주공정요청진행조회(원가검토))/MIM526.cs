...
// 작 성 자  :  한 미 애
// 작 성 일  :  2019-02-22
// 작성내용  :  외주공정요청조회 프로그램을 복사하여 생성함. 검색조건 및 
//              조회 그리드의 여러 항목들을 삭제하고 원가검토 항목 추가함.
// 수 정 일  :
// 수 정 자  :
// 수정내용  :
// 비    고  :
...
namespace MO.MIM526
{
    public partial class MIM526 : UIForm.FPCOMM1
    {
        ...
        #region Form Load 시
        private void MIM526_Load(object sender, System.EventArgs e)
        {
            //필수체크
            ...
            rdoReqStepAll.Checked = true;       // 구매요청진행단계 검색조건에서 '전체'를 기본값으로.
        }
        #endregion
        ...
        protected override void SearchExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1) == true)
            {
                ...
                try
                {
                    ...
                    if (rdoReqStepAll.Checked)
                        ReqStep = "ALL";
                    else if (rdoReqStepCnfm.Checked)
                        ReqStep = "REQCFM";
                    else if (rdoReqStepPO.Checked)
                        ReqStep = "PO";
                    else if (rdoReqStepPOCnfm.Checked)
                        ReqStep = "POCFM";
                    else if (rdoReqStepInv.Checked)
                        ReqStep = "INV";

                    string strQuery = "usp_MIM526 @pTYPE = 'S1'";
                    ...
                    strQuery += ", @pITEM_CD = '" + txtItemCd.Text + "'";               // 품목코드
                    strQuery += ", @pDELIVERY_DT_FR = '" + dtpDeliveryDtFr.Text + "'";  // 입고요구일FROM
                    strQuery += ", @pDELIVERY_DT_TO = '" + dtpDeliveryDtTo.Text + "'";  // 입고요구일TO
                    strQuery += ", @pREQ_STATUS = '" + txtReqStatus.Text + "'";         // 요청진행상태
                    //strQuery += ", @pREQ_DEPT_CD = '" + txtReqDeptCd.Text + "'";      // 2019.02.25. hma 수정: 화면에 보이지 않는 항목이므로 주석 처리함.
                    //strQuery += ", @pREQ_TYPE = '" + txtReqType.Text + "'";           // 2019.02.25. hma 수정: 화면에 보이지 않는 항목이므로 주석 처리함.
                    strQuery += ", @pREQ_ID = '" + txtReqId.Text + "'";                 // 요청담당
                    strQuery += ", @pREQ_NO = '" + txtReqNo.Text + "'";                 // 요청번호
                    strQuery += ", @pPO_NO = '" + txtPoNo.Text + "'";                   // 발주번호
                    strQuery += ", @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "'";
                    strQuery += ", @pWORKORDER_NO_FR = '" + txtWorkorder_No_Fr.Text + "'";  // 제조지시FROM
                    strQuery += ", @pWORKORDER_NO_TO = '" + txtWorkorder_No_To.Text + "'";  // 제조지시TO
                    strQuery += ", @pCUST_CD = '" + txtCustCd.Text + "'";       // 공급처
                    strQuery += ", @pPUR_DUTY= '" + txtPurDuty.Text + "'";      // 발주담당                  
                    strQuery += ", @pCLOSE_YN = '" + strCfmYn + "'";            // 마감여부
                    strQuery += ", @pPO_DT_FR = '" + dtpPODateFr.Text + "'";    // 발주일자FROM
                    strQuery += ", @pPO_DT_TO = '" + dtpPODateTo.Text + "'";    // 발주일자TO
                    strQuery += ", @pREQ_STEP = '" + ReqStep + "'";             // 요청진행단계
                    ...
                    strQuery += ", @pCFM = '" + strCfm + "'";               // 조회구분
                    strQuery += ", @pOUT_FLAG = '" + strOutFlag + "'";      // 외주상태(가공/전자, 피막/기타)
                    //strQuery += ", @pITEM_DIV = '" + Convert.ToString(cboItemDiv.SelectedValue) + "'";    // 2019.02.25. hma 수정: 화면에 보이지 않는 항목이므로 주석 처리함.

    ...
    public partial class MCC002 : UIForm.FPCOMM1
    {
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead.Length > 0)
                        {
                            ...
                            strQuery += ", @pCUST_NM = '"   + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "공급자명")].Text + "'";
                            ...
                            strQuery += ", @IMPORT_UNIT = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "품목단위")].Value + "'";       // 품목단위 추가 2015.08.12
                            ...
                            strQuery += ", @TAXPAYER_NM = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "납세의무자")].Value + "'";     // 납세의무자 추가 2015.08.12
        ...
        private void btnUpLoad_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    // 2018.01.12. hma 수정(Start): 윈도우 보안 업데이트후 문제가 생겨서 엑셀 업로드시 OLEDB 부분 수정함.
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "공급자명")].Text = ds.Tables[0].Rows[i][4].ToString().Trim();								//4.공급자명
                            ...
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "품목단위")].Value = ds.Tables[0].Rows[i][17].ToString().Trim();                  		    //21.품목단위 추가 2015.08.12
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "세번부호(HS코드)")].Text = ds.Tables[0].Rows[i][18].ToString().Trim();						//17.세번부호(HS코드)
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "세종=관세율(%)")].Value = Convert.ToDouble(ds.Tables[0].Rows[i][19].ToString().Trim());		//18.세종=관세율(%)
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "세종=관감면율(%)")].Value = Convert.ToDouble(ds.Tables[0].Rows[i][20].ToString().Trim());	//19.세종=관감면율(%)")
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "납세의무자")].Value = ds.Tables[0].Rows[i][21].ToString().Trim();                 		    //22.납세의무자 추가 2015.08.12
                            fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "농특세율(%)")].Value = Convert.ToDouble(ds.Tables[0].Rows[i][22].ToString().Trim());		//20.농특세율(%)

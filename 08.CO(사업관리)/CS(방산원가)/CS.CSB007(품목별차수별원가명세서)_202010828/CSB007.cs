            ...
            try
            {
                if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
                {
                    ...
                    if (fpSpread1.Sheets[0].Rows.Count > 0)
                    {
                        ...
                        {
                            if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "차수")].Text == "소계")
                            {
                                fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "차수"), i, fpSpread1.Sheets[0].Columns.Count - 1].BackColor = Color.LightPink;
                            }

                            if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "품목코드")].Text == "합계")
                            {
                                fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "품목코드"), i, fpSpread1.Sheets[0].Columns.Count - 1].BackColor = Color.LightBlue;
                            }
                        }
                    }
                }

...
namespace AG.ACG401
{
    public partial class ACG401 : UIForm.FPCOMM2
    {
        #region 변수선언
        ...
        #region Form Load 시
        private void ACG401_Load(object sender, System.EventArgs e)
        {
        ...
        protected override void NewExec()
        {
            ...
            dtpPaymentDtTo.Text = SystemBase.Base.ServerTime("YYMMDD");
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_ACG401  'S1'";
                    strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' "; 
                    strQuery += ", @pPAYMENT_DT_FROM = '" + dtpPaymentDtFr.Text + "' ";
                    strQuery += ", @pPAYMENT_DT_TO = '" + dtpPaymentDtTo.Text + "' ";
                    ...
                    // 2022.04.01. hma 추가(Start): 청산완료여부 검색조건 추가
                    if (rdoAmtAll.Checked == true)
                        strQuery += ", @pAMT_CLOSE_YN = '' ";
                    else if (rdoAmtClose.Checked == true)
                        strQuery += ", @pAMT_CLOSE_YN = 'Y' ";
                    else if (rdoAmtIng.Checked == true)
                        strQuery += ", @pAMT_CLOSE_YN = 'N' ";
                    // 2022.04.01. hma 추가(End)

                    // 2022.05.13. hma 추가(Start): 장기선급금제외여부 추가
                    if (chkLongTerm.Checked == true)
                        strQuery += ", @pLONG_TERM_EXCLU = 'Y' ";
                    else
                        strQuery += ", @pLONG_TERM_EXCLU = 'N' ";
                    // 2022.05.13. hma 추가(End)
        ...
        private void fpSpread2_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            try
            {
                ...
                if (intRow != -1) strSLIP_NO = fpSpread2.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx2, "결의전표번호")].Text;
                if (PreRow == intRow && PreRow != -1 && intRow != -1)   //현 Row에서 컬럼이동시는 조회 안되게
                {
                    this.Cursor = System.Windows.Forms.Cursors.Default;
                    return;
                }
                string strPRP_NO = fpSpread2.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx2, "선급금번호")].Text;
                
                DETAIL_SEARCH(strPRP_NO);
        ...
        private void fpSpread1_SelectionChanged(object sender, FarPoint.Win.Spread.SelectionChangedEventArgs e)
        {
            try
            {
                ...
                if(intRow != -1) strSLIP_NO = fpSpread1.Sheets[0].Cells[intRow, SystemBase.Base.GridHeadIndex(GHIdx1, "전표번호")].Text;
            }
            catch (Exception f)
            {
                MessageBox.Show(f.ToString(), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        #region 전표내역 조회
        private void DETAIL_SEARCH(string strPRP_NO)
        {
            this.Cursor = Cursors.WaitCursor;
            try
            {
                //상세조회 SQL
                string strQuery = " usp_ACG401  'S2'";
                strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                strQuery = strQuery + ", @pPRP_NO ='" + strPRP_NO + "' ";
        ...
        private void txtAcctCd_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txtAcctNm.Value = SystemBase.Base.CodeName("ACCT_CD", "ACCT_NM", "A_ACCT_CODE", txtAcctCd.Text, " AND CO_CD = '" + SystemBase.Base.gstrCOMCD + "' AND ENTRY_YN = 'Y' AND ACCT_TYPE = 'G1'");
            }
            catch (Exception f)
            {
                MessageBox.Show(f.ToString(), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnAcct_Click(object sender, EventArgs e)
        {
            try
            {
                string strQuery = " usp_A_COMMON @pTYPE = 'A030', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' , @pSPEC1 = 'Y', @pSPEC2 = 'G1' ";

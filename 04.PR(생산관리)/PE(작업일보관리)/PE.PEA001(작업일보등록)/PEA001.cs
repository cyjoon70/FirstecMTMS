...
namespace PE.PEA001
{
    public partial class PEA001 : UIForm.FPCOMM2_2
    {
        #region 변수선언
        bool chk = true;
        #endregion
        ...
        #region Form Load 시
        private void PEA001_Load(object sender, System.EventArgs e)
        {
            //필수체크
            ...
            dtpWorkDt.Text = SystemBase.Base.ServerTime("YYMMDD").ToString().Substring(0, 10);

            string Query = " usp_PEA001 @pTYPE = 'S3', @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'"; //간접시수(비가동코드) default 세팅
        ...
        {
            txtSPlantCd.Text = SystemBase.Base.gstrPLANT_CD;
        ...
        protected override void SaveExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox2))
            {
                ...
                try
                {
                    ...
                    string strSql = " usp_PEA001 '" + strMType + "'";
                    strSql += ", @pWORK_DAY_NO = '" + txtWorkDayNo.Text + "' ";
                    ...
                    if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false)) //그리드 필수체크
                    {
                        ...
                        for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                        {
                            ...
                            if (strHead.Length > 0)
                            {
                                ...
                                string strDSql = " usp_PEA001 '" + strGbn + "'";
                                ...
                                strDSql += ", @pJOB_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드")].Text.Trim() + "' ";
                    ...
                    if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread2, this.Name, "fpSpread2", false)) //그리드 필수체크
                    {
                        ...
                        for (int i = 0; i < fpSpread2.Sheets[0].Rows.Count; i++)
                        {
                            ...
                            if (strHead.Length > 0)
                            {
                                ...
                                string strISql = " usp_PEA001 '" + strGbn + "'";
        ...
        protected override void SearchExec()
        {
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                ...
                try
                {
                    ...
                    string strQuery = " usp_PEA001  @pTYPE = 'S1'";
                    ...
                    if (fpSpread1.Sheets[0].Rows.Count == 0 && fpSpread2.Sheets[0].Rows.Count == 0 && txtWorkDayNo.Text == "")
                    {
                        NewExec();
                    }
                }
        ...
        protected void SubSearch(string strWorkDayNo)
        {
            ...
            try
            {
                string strQuery = " usp_PEA001  @pTYPE = 'S4'";
        ...
        private void txtWorkDutyId_TextChanged(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (txtSWorkDayNo.Text == "" && txtWorkDutyNm.Text != "")
                {
                    DataTable dt = SystemBase.DbOpen.NoTranDataTable("SELECT TOP 1 WORK_DAY_NO FROM P_WORK_HDR(NOLOCK) WHERE WORK_DUTY = '" + txtWorkDutyId.Text + "' AND WORK_DT = '" + dtpWorkDt.Text + "'  AND CO_CD='" + SystemBase.Base.gstrCOMCD + "' ");

                    if (dt.Rows.Count > 0)
                    {
                        MessageBox.Show(dt.Rows[0][0].ToString() + "  :: 이미 등록된 작업일보가 있습니다. 조회 후 추가 등록 하십시오.", SystemBase.Base.MessageRtn("Z0003"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch
        ...
        private void PEA001_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            int UpCount = 0;
        ...
        private void fpSpread1_ButtonClicked(object sender, FarPoint.Win.Spread.EditorNotifyEventArgs e)
        {
            ...
            if (e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호_2"))
            {
                try
                {
                    ...
                    if (pu.DialogResult == DialogResult.OK)
                    {
                        ...
                        fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "오더수량")].Text = Msgs[12].ToString();
                        fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "오더단위")].Text = Msgs[16].ToString();
                        fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "사업")].Text = Msgs[22].ToString();
                        ...
                        txtOrderQty.Value = Msgs[12].ToString();
                        txtOrderUnit.Value = Msgs[16].ToString();
                        txtEntCd.Value = Msgs[22].ToString();
                    }
                }
                catch (Exception f)
                {
            ...
            //공정
            else if (e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "공정_2"))
            {
                if (fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text != "")
                {
                    try
                    {
                        string strQuery = " usp_PEA001 'P1', @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'";
                        ...
                        if (pu.DialogResult == DialogResult.OK)
                        {
                            ...
                            //작업,작업장 숨김필드
                            fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드")].Text = Msgs[1].ToString();
                            fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Text = Msgs[2].ToString();
                            fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "작업장")].Text = Msgs[8].ToString();
            ...
            // 공정조회
            else if (e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드_2"))
            {
                ...
                if (pu.DialogResult == DialogResult.OK)
                {
                    ...
                    fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드")].Text = Msgs[0].ToString(); //공정코드
                    fpSpread1.Sheets[0].Cells[e.Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Value = Msgs[1].ToString(); //공정명

                    UIForm.FPMake.fpChange(fpSpread1, e.Row);
                }
            }
            //설비자원
            else if (e.Column == SystemBase.Base.GridHeadIndex(GHIdx1, "설비자원_2"))
            {
                try
                {
        ...
        protected override void fpSpread1_ChangeEvent(int Row, int Column)
        {
            if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호"))
            {
                ...
                {
                    string strQuery = "usp_PEA001 'S6', @pWORKORDER_NO = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text + "' , @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                    ...
                    if (WorkDt.Rows.Count > 0)
                    {
                        ...
                        fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "품목명")].Text = WorkDt.Rows[0]["ITEM_NM"].ToString();
            ...
            else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "공정"))
            {
                if (fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text != "")
                {
                    ...
                    {
                        string ProcQuery = "usp_PEA001 'P3', @pWORKORDER_NO = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text + "', @pPROC_SEQ = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정")].Text + "', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                        DataTable ProcDt = SystemBase.DbOpen.NoTranDataTable(ProcQuery);

                        if (ProcDt.Rows.Count > 0)
                        {
                            fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드")].Text = ProcDt.Rows[0]["JOB_CD"].ToString();
                            fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Text = ProcDt.Rows[0]["JOB_NM"].ToString();
            ...
            {
                string strQuery = "";
                strQuery += " usp_PEA001 'P2' ";
                strQuery += " , @pLANG_CD='" + SystemBase.Base.gstrLangCd + "'";
                strQuery += " , @pJOB_CD='" + fpSpread1.Sheets[0].Cells[Row, Column].Text.Trim() + "'";
                strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    // 자원정보를 조회한다.
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드")].Text = dt.Rows[0]["MINOR_CD"].ToString();	//공정코드
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Text = dt.Rows[0]["CD_NM"].ToString();	//공정명

                    UIForm.FPMake.fpChange(fpSpread1, Row);
                }
                else
                {
                    fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정명")].Text = "";	//공정명
            ...
            else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "준비시간(분)"))
            {
                ...
                for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                {
                    ReadySum = ReadySum + Convert.ToInt32(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "준비시간(분)")].Value);
                }
                ...
                txtTotalSum.Value = Convert.ToString(ReadySum + InDirectSum + DirectSum);
            }
        }
        #endregion
        ...
        {
            string Query = "usp_PEA001 @pTYPE = 'S2', @pWORK_DAY_NO = '" + strWorkDayNo + "' ";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dtpWorkDt.Value = dt.Rows[0]["WORK_DT"].ToString().Substring(0, 10);
                ...
                for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                {
                    WorkTm = WorkTm + Convert.ToInt32(fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "실동시간(분)")].Value);
                }
        ...
        private void SubData(int Row)
        {
            ...
            txtJobCd.Value = fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정코드")].Text;
        ...
        private void TouchSearch(string ResCd, string WorkDt)
        {
            ...
            try
            {
                string strQuery = " usp_PEA001  @pTYPE = 'S5'";
        ...
        private void fpSpread1_EditChange(object sender, FarPoint.Win.Spread.EditorNotifyEventArgs e)
        {
            try
            {
                ...
                if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호"))
                {
                    if (fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text != "")
                    {
                        string strQuery = "usp_PEA001 'S6', @pWORKORDER_NO = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text + "' ";
                        strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";
                ...
                else if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "공정"))
                {
                    if (fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text != "")
                    {
                        if (fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "")].Text != "공정")
                        {
                            string ProcQuery = "usp_PEA001 'P3', @pWORKORDER_NO = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "제조오더번호")].Text + "', @pPROC_SEQ = '" + fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "공정")].Text + "' ";
                            ProcQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' "; 
                            DataTable ProcDt = SystemBase.DbOpen.NoTranDataTable(ProcQuery);

...
namespace QE.QRE008
{
    public partial class QRE008 : UIForm.FPCOMM1
    {
        #region 변수선언
        Thread th;
        ...
        #region Form Load시
        private void QRE008_Load(object sender, System.EventArgs e)
        { 
            //필수체크
        ...
        private void Grd_Set()
        {
            ...
            try
            {
                string strQuery = " usp_QRE008  @pTYPE = 'S2'";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        fpSpread1.Sheets[0].ColumnHeader.Cells[0, i + 11].Text = dt.Rows[i]["DEFECT_TYPE_NM"].ToString();

                        fpSpread1.Sheets[0].Columns[i + 11].CellType = num;
                        fpSpread1.Sheets[0].Columns[i + 11].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Right;
                        fpSpread1.Sheets[0].Columns[i + 11].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                        fpSpread1.Sheets[0].Columns[i + 11].Locked = true;
                        fpSpread1.Sheets[0].Columns[i + 11].Width = 60;
        ...
        protected override void NewExec()
        {
            ...
            //기타 세팅
            cboPlantCd.SelectedValue = SystemBase.Base.gstrPLANT_CD;
            dtpInspDtFr.Value = Convert.ToDateTime(SystemBase.Base.ServerTime("YYMMDD")).AddMonths(-1);
            dtpInspDtTo.Value = SystemBase.Base.ServerTime("YYMMDD");
        }
        #endregion
        ...
        protected override void SearchExec()
        {
            ...
            try
            {
                if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
                {
                    ...
                    if (rdoDivDo.Checked == true) { strDiv = "내자"; }
                    else if (rdoDivPo.Checked == true) { strDiv = "외자"; }
                    else { strDiv = ""; }

                    string strQuery = " usp_QRE008  @pTYPE = 'S1'";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            if (div != dt.Rows[i]["ENT_CD"].ToString()
                                || div1 != dt.Rows[i]["PROJECT_NO"].ToString())
                            {
                                fpSpread1.Sheets[0].RowCount = row_idx + 1;

                                fpSpread1.Sheets[0].Cells[row_idx, 1].Text = dt.Rows[i]["ENT_CD"].ToString();
                                fpSpread1.Sheets[0].Cells[row_idx, 2].Text = dt.Rows[i]["ENT_NM"].ToString();
                                fpSpread1.Sheets[0].Cells[row_idx, 3].Text = dt.Rows[i]["PROJECT_NO"].ToString();
                                fpSpread1.Sheets[0].Cells[row_idx, 4].Value = dt.Rows[i]["INSP_QTY"];
                                fpSpread1.Sheets[0].Cells[row_idx, 5].Value = dt.Rows[i]["GOOD_QTY"];
                                fpSpread1.Sheets[0].Cells[row_idx, 6].Value = dt.Rows[i]["DEFECT_QTY"];				
                                fpSpread1.Sheets[0].Cells[row_idx, 7].Value = dt.Rows[i]["INSP_RATE"];
                                fpSpread1.Sheets[0].Cells[row_idx, 8].Value = dt.Rows[i]["손폐비용"];
                                fpSpread1.Sheets[0].Cells[row_idx, 9].Value = dt.Rows[i]["손폐율"];

                                div = dt.Rows[i]["ENT_CD"].ToString();
                                div1 = dt.Rows[i]["PROJECT_NO"].ToString();
                                row_idx++;
                            }
        ...
        private void btnPrivew_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                string strQuery = " usp_QRE008  @pTYPE = 'R1'";
                strQuery += ", @pPROJECT_NO = '" + txtProjectNo.Text + "'";
                strQuery += ", @pPLANT_CD = '" + cboPlantCd.SelectedValue.ToString() + "'";
                strQuery += ", @pENT_CD = '" + txtEntCd.Text + "'";
                strQuery += ", @pINSP_DT_FR = '" + dtpInspDtFr.Text + "'";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    excel.SetCell(24, 1, "사업코드");
                    excel.SetCell(24, 3, "사업명");

                    string strQuery1 = " usp_QRE008  @pTYPE = 'R2'";
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        ...
                        {
                            excel.SetCell(25 + i, 1, dt1.Rows[i]["ENT_CD"].ToString());
                            excel.SetCell(25 + i, 3, dt1.Rows[i]["ENT_NM"].ToString());
                    ...
                    string strQuery2 = " usp_QRE008  @pTYPE = 'R3'";

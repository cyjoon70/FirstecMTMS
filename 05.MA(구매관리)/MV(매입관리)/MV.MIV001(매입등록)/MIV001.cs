...
namespace MV.MIV001
{
    public partial class MIV001 : UIForm.FPCOMM2
    {
        ...
        private void MIV001_Load(object sender, System.EventArgs e)
        {
            ...
            lnkJump1.Text = "확정전표상신";         // 2021.12.17. hma 추가: 화면에 보여지는 링크명
            strJumpFileName1 = "AD.ACD001.ACD001";  // 2021.12.17. hma 추가: 호출할 화면명
            lnkJump2.Text = "반제전표상신";         // 2022.01.21. hma 추가: 화면에 보여지는 링크명
            strJumpFileName2 = "AD.ACD001.ACD001";  // 2022.01.21. hma 추가: 호출할 화면명
        ...
        private void SubSearch(string strCode)
        {
            ...
            try
            {
                ...
                {
                    if (dt.Rows[0]["CONFIRM_YN"].ToString() == "Y")     // 확정한 건이면
                    {
                    ...
                    {
                        rdoCfm_N.Checked = true;
                        // 2022.01.21. hma 추가(Start): 미확정 상태이지만 반제전표번호가 없거나, 반제전표번호가 있으면서 반제전표상태가 승인이고 반제승인여부가 Y인 경우
                        if ((txtMSlipNo.Text == "") ||
                           (txtMSlipNo.Text != "" &&
                        ...
                        {
                            // 2022.02.12. hma 수정(Start): 확정 버튼이 활성화 되어있을 경우에만 참조 버튼 활성화.
                            //btnRef1.Enabled = true;     // 매입대상참조(외주)
                            if (btnConfirmOk.Enabled == true)
                                btnRef1.Enabled = true;     // 매입대상참조(외주)
                            else
                                btnRef1.Enabled = false;     // 매입대상참조(외주)
                            // 2022.02.12. hma 수정(End)
                ...
                if (dt.Rows[0]["CONFIRM_YN"].ToString() == "Y" || txtIvType.Text == "IIV"
                    // 2022.01.23. hma 추가(Start): 미확정 상태이지만 반제전표를 생성해서 승인상태가 아니면
                     || (dt.Rows[0]["CONFIRM_YN"].ToString() != "Y" && dt.Rows[0]["MINUS_SLIP_NO"].ToString() != "" && 

...
namespace QP.QPA004
{
    public partial class QPA004 : UIForm.FPCOMM3
    {
        #region 변수선언
        ...
        public QPA004(string param1, string param2, string param3, string param4)
        {
            //
        ...
        #region Form Load시
        private void QPA004_Load(object sender, System.EventArgs e)
        {
            //필수체크
            ...
            //그리드콤보박스 세팅
            G1Etc[SystemBase.Base.GridHeadIndex(GHIdx1, "불량원인")] = SystemBase.ComboMake.ComboOnGrid("usp_Q_COMMON @pType='Q060', @pCODE = 'P', @pCO_CD='" + SystemBase.Base.gstrCOMCD + "'", 0);//불량원인
            ...
            strJumpFileName1 = "QP.QPA011.QPA011"; //호출할 화면명

            lnkJump2.Text = "검사항목등록";  //화면에 보여지는 링크명
            strJumpFileName2 = "QP.QPA001.QPA001"; //호출할 화면명
        ...
        protected override void RowInsExec()
        {
            if (fpSpread2.Sheets[0].Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    fpSpread1.Sheets[0].Cells[fpSpread1.Sheets[0].ActiveRowIndex, SystemBase.Base.GridHeadIndex(GHIdx1, "검사레벨")].Text = fpSpread2.Sheets[0].Cells[iRow2, SystemBase.Base.GridHeadIndex(GHIdx2, "검사레벨")].Text;
                }
                catch (Exception f)
                {
                    SystemBase.Loggers.Log(this.Name, f.ToString());
        ...
        private void Grid_Search()
        {
            ...
            try
            {
                ...
                {
                    string strQuery = " usp_QPA004  @pTYPE = 'S1'";
                    ...
                    {
                        fpSpread3.Search(0, strInspReqNo, true, true, true, true, 0, SystemBase.Base.GridHeadIndex(GHIdx3, "검사의뢰번호"), ref SearchRow, ref ShowColumn);

                        if (SearchRow < 0)
                        { SearchRow = 0; }

                        SubSearch1(SearchRow);                       
                    }
                    else
                    {
                        SystemBase.Validation.GroupBox_Reset(groupBox2);
        ...
        #region 상세정보 조회
        private void SubSearch1(int iRow)
        {
            this.Cursor = Cursors.WaitCursor;
            ...
            try
            {
                string strQuery = " usp_QPA004  @pTYPE = 'S2'";
        ...
        #region fpSpread2 그리드 선택시 검사치정보조회
        ...
        #region 검사치정보조회
        private void SubSearch2(int iRow)
        {
            this.Cursor = Cursors.WaitCursor;
           
            try
            {
                string strQuery = " usp_QPA004  @pTYPE = 'S3'";
                strQuery += ", @pINSP_REQ_NO = '" + txtInspReqNo.Text + "'";
                strQuery += ", @pLANG_CD = '" + SystemBase.Base.gstrLangCd + "'";
                strQuery += ", @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' ";

                strKey = fpSpread2.Sheets[0].Cells[iRow, SystemBase.Base.GridHeadIndex(GHIdx2, "KEY")].Text;
                        ...
                        if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false))
                        {
                            ...
                            for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                            {
                                ...
                                if (strHead.Length > 0)
                                {
                                    ...
                                    if (strGbn != "")
                                    {
                                        string strSql = " usp_QPA004 '" + strGbn + "'";
                                        ...
                                        strSql += ", @pDEFECT_TYPE_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "불량유형")].Text + "' ";
                                        strSql += ", @pDEFECT_CAUSE_CD = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "불량원인")].Value + "' ";
            ...
            if (Column == SystemBase.Base.GridHeadIndex(GHIdx1, "불량수"))
            {
                if (fpSpread1.Sheets[0].Rows.Count > 0)
                {
                        ...
                        < Convert.ToDouble(fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "불량수")].Value))
                    {
                        ...
                        fpSpread1.Sheets[0].Cells[Row, SystemBase.Base.GridHeadIndex(GHIdx1, "불량수")].Value = strDefectQty;
                    }
                }
            }
        }

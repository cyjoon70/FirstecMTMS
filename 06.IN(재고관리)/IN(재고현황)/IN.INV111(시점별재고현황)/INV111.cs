...
namespace IN.INV111
{
    public partial class INV111 : UIForm.FPCOMM1
    {
        #region 변수선언
        int SDown = 1;		// 조회 횟수
        int AddRow = 100;
        ...
        #region Form Load 시
        private void INV111_Load(object sender, System.EventArgs e)
        {
            //필수체크
            ...
            SystemBase.ComboMake.C1Combo(cboItemAcct, "usp_B_COMMON @pTYPE='COMM', @pCODE = 'B036', @pLANG_CD='" + SystemBase.Base.gstrLangCd + "', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "'", 0); // 2017.12.12. hma 추가:품목계정
        ...
        protected override void SearchExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                ...
                try
                {
                    // 2017.12.12. hma 추가(Start): 재고구분 체크
                    string strStockFlag = "";
                    if (rdoY.Checked == true)
                        strStockFlag = "Y";
                    else
                        strStockFlag = "N";
                    // 2017.12.12. hma 추가(End)

                    string strQuery = " usp_INV111 'S1'";
                    ...
                    strQuery += ", @pITEM_ACCT ='" + cboItemAcct.SelectedValue.ToString() + "'";    // 2017.12.12. hma 추가: 품목계정
                    strQuery += ", @pSTOCK_FLAG ='" + strStockFlag + "'";                           // 2017.12.12. hma 추가: 품목구분(Y:0이상만,N:전체)
        ...
        private void btnItemFr_Click(object sender, EventArgs e)
        {
            try
            {
                WNDW001 pu1 = new WNDW001(txtItemCdFr.Text, cboPlantCd.SelectedValue.ToString());
                pu1.ShowDialog();
        ...
        private void btnItemTo_Click(object sender, EventArgs e)
        {
            try
            {
                WNDW001 pu1 = new WNDW001(txtItemCdTo.Text, cboPlantCd.SelectedValue.ToString());
                pu1.ShowDialog();
                if (pu1.DialogResult == DialogResult.OK)
                {
                    string[] Msgs = pu1.ReturnVal;
        ...
        private void INV111_Activated(object sender, System.EventArgs e)
        {
            if (form_act_chk == false) dtpTranDt.Focus();
        }

        private void INV111_Deactivate(object sender, System.EventArgs e)
        {
            form_act_chk = true;
        }

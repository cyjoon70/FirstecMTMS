...
namespace DA.DAA002
{
    public partial class DAA002P17 : UIForm.FPCOMM1
    {
        #region 변수선언
        int iPK_SEQ = 0;
        ...
        #region DAA002P17 
        public DAA002P17(int PK_SEQ, string MNUF_CODE, string ORDR_YEAR, string DPRT_CODE, string DCSN_NUMB, string CALC_DEGR, string FormId)
        {
            InitializeComponent();
        ...
        #region DAA002P17_Load
        private void DAA002P17_Load(object sender, EventArgs e)
        {
            ...
            this.Text = SystemBase.Base.GetMenuTree(strFormId) + " > 원가투입이력";
        }
        #endregion
            ...
            try
            {
                if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))  //필수조회조건 체크
                {
                    ...
                    if (fpSpread1.Sheets[0].RowCount > 0)
                    {
                        ...
                        {
                            if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "부품관리번호")].Text == "ERROR")
                            {
        ...
        #region DAA002P17_Shown 조회
        private void DAA002P17_Shown(object sender, EventArgs e)
        {
            SearchExec();
        }
        #endregion

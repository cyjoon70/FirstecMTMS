...
namespace AG.ACG302
{
    public partial class ACG302 : UIForm.FPCOMM1 
    {
        string strREORG_ID = "";
        public ACG302()
        {
        ...
        #region Form Load 시
        private void ACG302_Load(object sender, System.EventArgs e)
        {
            SystemBase.Validation.GroupBox_Setting(groupBox1);//필수 적용
            SystemBase.Validation.GroupBox_Setting(groupBox2);//필수 적용
            SystemBase.ComboMake.C1Combo(cboNoteKind, "usp_B_COMMON @pTYPE='COMM', @pCODE = 'A507', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "'", 9);   //어음구분
        ...
        protected override void SaveExec()
        {
            ...
            if (fpSpread1.Sheets[0].Rows.Count > 0)
            {
                if (fpSpread1.Sheets[0].Cells[0, SystemBase.Base.GridHeadIndex(GHIdx1, "조회TYPE")].Text == "R")
                {
                    ...
                    if (txtChargeAmt.Text != "" && txtChargeAmt.Text != "0" && txtChargeAcctCd.Text == "")
                    {
                        this.Cursor = Cursors.Default;
                        MessageBox.Show("수수료계정을 입력하세요.", SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txtChargeAcctCd.Focus();
                        return;
                    }
                    else if (txtChargeAmt.Text == "" && txtChargeAcctCd.Text != "")
                    {
                        this.Cursor = Cursors.Default;
                        MessageBox.Show("수수료금액을 입력하세요.", SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txtChargeAmt.Focus();
                        return;
                    }
                }
            ...
            try
            {
                if (SystemBase.Validation.FPGrid_SaveCheck(fpSpread1, this.Name, "fpSpread1", false) == true)
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        {
                            string strSql = " usp_ACG302 'I1'";
                            strSql = strSql + ", @pCO_CD  = '" + SystemBase.Base.gstrCOMCD + "'";
                            strSql = strSql + ", @pACT_TYPE = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "조회TYPE")].Text + "' ";
                            strSql = strSql + ", @pNOTE_KIND = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "어음구분")].Text + "' ";
                            if (fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "조회TYPE")].Text == "C")//취소
                            {
                            ...
                            if(txtChargeAmt.Text.Replace(",","") != "")
                                strSql = strSql + ", @pCHARGE_AMT = '" + txtChargeAmt.Text.Replace(",", "") + "' ";
                            strSql = strSql + ", @pCHARGE_ACCT_CD = '" + txtChargeAcctCd.Text + "' ";
        ...
        //수수료계정
        private void txtChargeAcctCd_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txtChargeAcctNm.Value = SystemBase.Base.CodeName("ACCT_CD", "ACCT_NM", "A_ACCT_CODE", txtChargeAcctCd.Text, " AND CO_CD = '" + SystemBase.Base.gstrCOMCD + "' AND ENTRY_YN = 'Y'");
            }
            catch (Exception f)
            {
                MessageBox.Show(f.ToString(), SystemBase.Base.MessageRtn("Z0002"), MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        //수수료계정
        private void btnChargeAcct_Click(object sender, EventArgs e)
        {
            try
            {
                string strQuery = " usp_A_COMMON @pTYPE = 'A030', @pCO_CD = '" + SystemBase.Base.gstrCOMCD + "' , @pSPEC1 = 'Y' ";
                string[] strWhere = new string[] { "@pCODE", "@pNAME" };
                string[] strSearch = new string[] { txtChargeAcctCd.Text, "" };
                UIForm.FPPOPUP pu = new UIForm.FPPOPUP("P00110", strQuery, strWhere, strSearch, new int[] { 0, 1 }, "계정코드 조회");
                pu.Width = 800;
                pu.ShowDialog();
                if (pu.DialogResult == DialogResult.OK)
                {
                    Regex rx1 = new Regex("#");
                    string[] Msgs = rx1.Split(pu.ReturnVal.ToString());
                    txtChargeAcctCd.Value = Msgs[0].ToString();
                    txtChargeAcctNm.Value = Msgs[1].ToString();
                }
            }
            catch (Exception f)
            {

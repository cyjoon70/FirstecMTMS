...
namespace AD.ACD001
{
    public partial class ACD001P10 : UIForm.Buttons
    {
        ...
        public ACD001P10(string SLIP_NO, string TASK_TYPE, string USER_ID, FarPoint.Win.Spread.FpSpread ASSIGN_GRID)
        {
            strSlipNo = SLIP_NO;
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < fpAssignGrid.Sheets[0].Rows.Count; i++)
                        {
                            ...
                            if (strHead.Length > 0)
                            {
                                ...
                                strSql = strSql + ", @pSLIP_NO = '" + txtSlipNo.Text + "'";
                                strSql = strSql + ", @pTASK_NO = '" + strNewAssignNo + "'";
                                strSql = strSql + ", @pTASK_SEQ = '" + fpAssignGrid.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "요청SEQ")].Text + "'";
                                strSql = strSql + ", @pASSIGN_ID = '" + fpAssignGrid.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "결재자")].Text + "'";
                                strSql = strSql + ", @pASSIGN_SEQ = '" + fpAssignGrid.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "결재순서")].Text + "'";
                                strSql = strSql + ", @pASSIGN_TYPE = '" + fpAssignGrid.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "결재단계")].Value.ToString() + "'";
                                strSql = strSql + ", @pASSIGN_OWNER = '" + fpAssignGrid.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "결재구분")].Value.ToString() + "'";
                                strSql = strSql + ", @pUP_ID = '" + SystemBase.Base.gstrUserID + "'";

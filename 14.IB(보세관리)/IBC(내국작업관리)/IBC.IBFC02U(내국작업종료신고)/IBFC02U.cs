...
{
    public partial class IBFC02U : UIForm.FPCOMM1
    {
        #region 생성자
        public IBFC02U()
        {
        ...
        #region Form Load 시
        private void IBFC02U_Load(object sender, System.EventArgs e)
        {
            //필수체크
        ...
        protected override void PrintExec()
        {
            ...
            string RptName = @"Report\" + "IBFC22P.rpt";
            ...
            RptParmValue[5] = txtDEGREE.Text;
            RptParmValue[6] = txtNO.Text;
        ...
        protected override void SearchExec()
        {
            ...
            if (SystemBase.Validation.GroupBox_SaveSearchValidation(groupBox1))
            {
                try
                {
                    ...
                    if (rdoNew.Checked == true) strQuery += " S21,";
                    else
                    {
                        if (rdoClose.Checked == true) strQuery += " S22,";
                        else strQuery += " S23,";
                    }
                    ...
                    strQuery = strQuery + " @pFINISH_DEGREE = '" + txtDEGREE.Text + "',";
                ...
                try
                {
                    ...
                    for (int i = 0; i < fpSpread1.Sheets[0].Rows.Count; i++)
                    {
                        ...
                        if (strHead.Length > 0)
                        {
                            switch (strHead)
                            {
                                case "U": strGbn = "U2"; break;   //수정
                                //								case "D": strGbn = "D1"; break;   //삭제
                                case "I": strGbn = "U2"; break;   //입력
                            ...
                            strQuery = strQuery + ", @pFINISH_NO = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "내국작업허가번호")].Text.Trim() + "'";
                            strQuery = strQuery + ", @pOUT_PLAN_DT = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "반출예정일")].Text + "'";
                            strQuery = strQuery + ", @pOUT_REASON = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "반출사유")].Text.Trim() + "'";
                            strQuery = strQuery + ", @pFINISH_PERMISSION_NO = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "종료허가번호")].Text.Trim() + "'";
                            strQuery = strQuery + ", @pFINISH_DEGREE = '" + fpSpread1.Sheets[0].Cells[i, SystemBase.Base.GridHeadIndex(GHIdx1, "종료차수")].Text.Trim() + "'";
        ...
        private void butRef_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                IBFC02P frm = new IBFC02P(fpSpread1, txtNO.Text, txtDEGREE);
                frm.ShowDialog();
            }
            catch (Exception f)
            {
